...
jQuery( document ).ready(
	function() {
		...
		catamaran_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="catamaran_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.catamaran_post_editor' );
						if ( link.length > 0 ) {
							catamaran_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.catamaran_post_editor', function(e) {
						catamaran_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('catamaran_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function catamaran_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
