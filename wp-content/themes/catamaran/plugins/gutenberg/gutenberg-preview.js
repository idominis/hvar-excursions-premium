...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			catamaran_create_observer( 'catamaran-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				catamaran_gutenberg_editor_init();
			} );
		} else {
		 	catamaran_gutenberg_editor_init();
		}
	...
	function catamaran_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'catamaran_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'catamaran_inited' ) ) {
	...
	// Init on page load
	function catamaran_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = catamaran_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + CATAMARAN_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + CATAMARAN_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in CATAMARAN_STORAGE['override_classes'] ) {
				$obj.addClass( CATAMARAN_STORAGE['override_classes'][i].replace( '%s', CATAMARAN_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( catamaran_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && CATAMARAN_STORAGE['sidebar_position'] != 'hide' ) {
				if ( catamaran_apply_filters( 'catamaran_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'catamaran_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( catamaran_apply_filters( 'catamaran_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				catamaran_remove_observer( 'catamaran-check-editor-styles-wrapper' );
				catamaran_create_observer( 'catamaran-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'catamaran_inited' );
	}
} );
