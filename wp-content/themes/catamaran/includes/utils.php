...
if ( ! function_exists( 'catamaran_array_get_first' ) ) {
	/**
	 ...
	 */
	function catamaran_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'catamaran_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function catamaran_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'catamaran_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function catamaran_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'catamaran_array_from_list' ) ) {
	/**
	 ...
	 */
	function catamaran_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'catamaran_array_slice' ) ) {
	/**
	 ...
	 */
	function catamaran_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'catamaran_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * catamaran_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * catamaran_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function catamaran_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'catamaran_array_insert_after' ) ) {
	/**
	 ...
	 */
	function catamaran_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = catamaran_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: catamaran_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'catamaran_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function catamaran_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'catamaran_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function catamaran_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'catamaran_array_str_replace' ) ) {
	/**
	 ...
	 */
	function catamaran_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = catamaran_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'catamaran_generate_id' ) ) {
	/**
	 ...
	 */
	function catamaran_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'catamaran_get_tag' ) ) {
	/**
	 ...
	 */
	function catamaran_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'catamaran_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function catamaran_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'catamaran_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function catamaran_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( catamaran_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'catamaran_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function catamaran_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'catamaran_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function catamaran_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = catamaran_fga( $css );
...
if ( ! function_exists( 'catamaran_get_column_class' ) ) {
	/**
	 ...
	 */
	function catamaran_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'catamaran_get_svg_from_file' ) ) {
	/**
	 ...
	 */
	function catamaran_get_svg_from_file( $svg ) {
		if ( catamaran_is_url( $svg ) && ! catamaran_is_external_url( $svg ) ) {
			$svg_path = catamaran_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
		$content = catamaran_fgc( $svg );
		preg_match( "#<\s*?svg\b[^>]*>(.*?)</svg\b[^>]*>#is", $content, $matches );
		return ! empty( $matches[0]) ? str_replace( array( "\r", "\n" ), array( '', ' ' ), $matches[0] ) : '';
	}
}

if ( ! function_exists( 'catamaran_external_links_target' ) ) {
	/**
	 ...
	 */
	function catamaran_external_links_target( $admin = false ) {
...
if ( ! function_exists( 'catamaran_stripslashes' ) ) {
	/**
	 ...
	 */
	function catamaran_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
		...
		if ( is_array( $val ) ) {
			foreach ( $val as $k => $v ) {
				$val[ $k ] = catamaran_stripslashes( $v );
			}
		} else {
...
if ( ! function_exists( 'catamaran_get_value_gp' ) ) {
	/**
	 ...
	 */
	function catamaran_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'catamaran_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function catamaran_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'catamaran_get_value_gps' ) ) {
	/**
	 ...
	 */
	function catamaran_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'catamaran_get_session_value' ) ) {
	/**
	 ...
	 */
	function catamaran_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'catamaran_set_session_value' ) ) {
	/**
	 ...
	 */
	function catamaran_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'catamaran_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function catamaran_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'catamaran_get_cookie' ) ) {
	/**
	 ...
	 */
	function catamaran_get_cookie( $name, $defa = '' ) {
		return catamaran_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'catamaran_set_cookie' ) ) {
	/**
	 ...
	 */
	function catamaran_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'catamaran_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//catamaran_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//catamaran_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'catamaran' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'catamaran_hex2rgb' ) ) {
	/**
	 ...
	 */
	function catamaran_hex2rgb( $hex ) {
...
if ( ! function_exists( 'catamaran_hex2rgba' ) ) {
	/**
	 ...
	 */
	function catamaran_hex2rgba( $hex, $alpha ) {
		$rgb = catamaran_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'catamaran_rgba2hex' ) ) {
	/**
	 ...
	 */
	function catamaran_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'catamaran_hex2hsb' ) ) {
	/**
	 ...
	 */
	function catamaran_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = catamaran_rgb2hsb( catamaran_hex2rgb( $hex ) );
...
if ( ! function_exists( 'catamaran_rgb2hsb' ) ) {
	/**
	 ...
	 * @return int[]      An array with HSB components in the format: ['h' => hue_value, 's' => saturation_value, 'b' => brightness_value].
	 */
	function catamaran_rgb2hsb( $rgb ) {
		$hsb      = array();
...
if ( ! function_exists( 'catamaran_hsb2rgb' ) ) {
	/**
	 ...
	 * @return int[]      An array with RGB components in the format: ['r' => red_value, 'g' => green_value, 'b' => blue_value].
	 */
	function catamaran_hsb2rgb( $hsb ) {
		$rgb = array();
		$h   = round( $hsb['h'] );
...
if ( ! function_exists( 'catamaran_rgb2hex' ) ) {
	/**
	 ...
	 */
	function catamaran_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'catamaran_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function catamaran_hsb2hex( $hsb ) {
		return catamaran_rgb2hex( catamaran_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'catamaran_date_to_sql' ) ) {
	/**
	 ...
	 */
	function catamaran_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'catamaran_format_price' ) ) {
	/**
	 ...
	 */
	function catamaran_format_price( $price ) {
...
if ( ! function_exists( 'catamaran_num2size' ) ) {
	/**
	 ...
	 */
	function catamaran_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'catamaran_size2num' ) ) {
	/**
	 ...
	 */
	function catamaran_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'catamaran_parse_num' ) ) {
	/**
	 ...
	 */
	function catamaran_parse_num( $str ) {
...
if ( ! function_exists( 'catamaran_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function catamaran_prepare_macros( $str ) {
...
if ( ! function_exists( 'catamaran_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function catamaran_remove_macros( $str ) {
		return empty( $str ) ? '' : str_replace(
										array( '{{', '}}', '((', '))', '||' ),
										array( '', '', '', '', ' ' ),
										$str
...
if ( ! function_exists( 'catamaran_is_on' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_is_off' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_is_inherit' ) ) {
	/**
	 ...
	 */
	function catamaran_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'catamaran_strshort' ) ) {
	/**
	 ...
	 */
	function catamaran_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = catamaran_strip_tags( $str );
...
if ( ! function_exists( 'catamaran_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function catamaran_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'catamaran_strip_tags' ) ) {
	/**
	 ...
	 */
	function catamaran_strip_tags( $str ) {
...
if ( ! function_exists( 'catamaran_excerpt' ) ) {
	/**
	 ...
	 */
	function catamaran_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return catamaran_strwords( catamaran_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'catamaran_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function catamaran_unserialize_recover( $str ) {
...
if ( ! function_exists( 'catamaran_unserialize' ) ) {
	/**
	 ...
	 */
	function catamaran_unserialize( $str, $disable_objects = false ) {
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/i', $str, $matches )
				...
				try {
					...
					if ( false === $data ) {
						$data = @unserialize( catamaran_unserialize_recover( $str ) );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
						$data = @unserialize( catamaran_unserialize_recover( $str2 ) );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'catamaran_str_replace' ) ) {
	/**
	 ...
	 */
	function catamaran_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = catamaran_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( catamaran_str_replace( $from, $to, catamaran_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'catamaran_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function catamaran_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'catamaran_tags_count' ) ) {
	/**
	 ...
	 */
	function catamaran_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'catamaran_getimagesize' ) ) {
	/**
	 ...
	 */
	function catamaran_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = catamaran_is_url( $url ) ? catamaran_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'catamaran_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function catamaran_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = catamaran_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'catamaran_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function catamaran_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = catamaran_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = catamaran_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = catamaran_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = catamaran_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'catamaran_get_thumb_size' ) ) {
	 ...
	 */
	function catamaran_get_thumb_size( $ts ) {
		$thumb_sizes = catamaran_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "catamaran-thumb-{$ts}" ] ) ) {
			$ts = 'catamaran-thumb-' . $ts;
		}
		$retina = catamaran_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'catamaran_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'catamaran_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'catamaran-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'catamaran_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'catamaran-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'catamaran_get_closest_thumb_size' ) ) {
	 ...
	 */
	function catamaran_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'catamaran_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function catamaran_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = catamaran_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'catamaran_get_post_image' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'catamaran_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function catamaran_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = catamaran_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = catamaran_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = catamaran_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( catamaran_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = catamaran_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'catamaran_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function catamaran_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'catamaran_get_post_audio_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function catamaran_get_post_audio_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_audio_list_first' ) ? trx_addons_get_post_audio_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'catamaran_get_post_video' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function catamaran_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = catamaran_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = catamaran_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'catamaran_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'catamaran_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function catamaran_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'catamaran_get_post_video_list_first' ) ) {
	/**
	 ...
	 *                     or the URL only
	 */
	function catamaran_get_post_video_list_first( $src = false, $args = array() ) {
		return function_exists( 'trx_addons_get_post_video_list_first' ) ? trx_addons_get_post_video_list_first( $src, $args ) : '';
	}
}

if ( ! function_exists( 'catamaran_get_post_iframe' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function catamaran_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = catamaran_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = catamaran_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'catamaran_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'catamaran_get_embed_video' ) ) {
	/**
	 ...
	 */
	function catamaran_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
		} else if ( catamaran_is_from_uploads( $video ) ) {
			$embed_video  = '<video'
								. ' src="' . esc_url( $video ) . '"'
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = catamaran_add_to_url( $video, catamaran_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'catamaran_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'catamaran_filter_video_width', 325, 'tiktok' ) : catamaran_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'catamaran_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'catamaran_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function catamaran_get_embed_video_tag_name() {
...
if ( ! function_exists( 'catamaran_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function catamaran_make_video_autoplay( $video, $muted = false ) {
			...
			if ( false !== $pos ) {
				if ( preg_match( '/(<' . esc_html( $tag ) . '[^>]*src=[\'"])([^\'"]+)([\'"][^>]*>)(.*)/is', $video, $matches ) ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = catamaran_add_to_url(
							$matches[2],
							array_merge(
								array(
								...
								$muted
									? array_merge(
										catamaran_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'catamaran_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function catamaran_is_from_uploads( $url ) {
		$url          = catamaran_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = catamaran_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'catamaran_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function catamaran_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'catamaran_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function catamaran_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'catamaran_fpc' ) ) {
	/**
	 ...
	 */
	function catamaran_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				return trx_addons_fpc( $file, $data, $flag );
			} else {
				$file = catamaran_prepare_path( $file );
...
if ( ! function_exists( 'catamaran_fgc' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_fga' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_mkdir' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_unlink' ) ) {
	/**
	 ...
	 */
	function catamaran_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
...
if ( ! function_exists( 'catamaran_copy' ) ) {
	/**
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				if ( is_dir( $src ) ) {
					...
					foreach ( $files as $file ) {
						if ( $file != "." && $file != ".." ) {
							catamaran_copy( "$src/$file", "$dst/$file" );
						}
					}
					return true;
...
if ( ! function_exists( 'catamaran_unzip_file' ) ) {
	/**
	 ...
	 * @return bool         Return true if no error occurs.
	 */
	function catamaran_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'catamaran_retrieve_json' ) ) {
	/**
...
if ( ! function_exists( 'catamaran_esc' ) ) {
	 ...
	 */
	function catamaran_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'catamaran_prepare_path' ) ) {
	/**
	 ...
	 */
	function catamaran_prepare_path( $path ) {
		return str_replace( array( '\\', '/' ), defined( 'DIRECTORY_SEPARATOR' ) ? DIRECTORY_SEPARATOR : '/', $path );
	}
}

if ( ! function_exists( 'catamaran_check_min_file' ) ) {
	/**
	 ...
	 */
	function catamaran_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && catamaran_is_off( catamaran_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && catamaran_is_off( catamaran_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'catamaran_get_file_dir' ) ) {
	/**
	 ...
	 */
	function catamaran_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! CATAMARAN_ALLOW_SKINS && ! catamaran_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'catamaran_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( CATAMARAN_CHILD_DIR != CATAMARAN_THEME_DIR && file_exists( CATAMARAN_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? CATAMARAN_CHILD_URL : CATAMARAN_CHILD_DIR ) . catamaran_check_min_file( $file, CATAMARAN_CHILD_DIR );
			} elseif ( file_exists( CATAMARAN_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? CATAMARAN_THEME_URL : CATAMARAN_THEME_DIR ) . catamaran_check_min_file( $file, CATAMARAN_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'catamaran_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function catamaran_get_file_url( $file ) {
		return catamaran_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'catamaran_get_file_ext' ) ) {
	/**
	 ...
	 */
	function catamaran_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'catamaran_get_file_name' ) ) {
	/**
	 ...
	 */
	function catamaran_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'catamaran_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function catamaran_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'catamaran_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( CATAMARAN_CHILD_DIR != CATAMARAN_THEME_DIR && is_dir( CATAMARAN_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? CATAMARAN_CHILD_URL : CATAMARAN_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( CATAMARAN_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? CATAMARAN_THEME_URL : CATAMARAN_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'catamaran_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'catamaran_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function catamaran_get_folder_url( $folder ) {
		return catamaran_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'catamaran_merge_js' ) ) {
	/**
	 ...
	 */
	function catamaran_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= catamaran_fgc( catamaran_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = catamaran_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = catamaran_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
					$file_dir = str_replace( '.js', '-full.js', $file_dir );
				}
			}
			catamaran_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'catamaran' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'catamaran_filter_js_output', apply_filters( 'catamaran_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'catamaran_merge_css' ) ) {
	/**
	 ...
	 */
	function catamaran_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'catamaran_filter_responsive_sizes', catamaran_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = catamaran_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = catamaran_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'catamaran_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'catamaran_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'catamaran' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'catamaran' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = catamaran_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			catamaran_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'catamaran') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'catamaran_filter_css_output', apply_filters( 'catamaran_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'catamaran_filter_merge_css' ) ) {
	add_filter( 'catamaran_filter_merge_css', 'catamaran_filter_merge_css' );
	/**
	 * Replace all relative paths in the merged CSS-file to resolve url() attributes.
	 *
	 * @hooked 'catamaran_filter_merge_css'
	 *
	 * @param string $css  A merged CSS content to resolve paths.
	 *
	 * @return string      A processed CSS content
	 */
	function catamaran_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'catamaran_merge_sass' ) ) {
	/**
	 ...
	 */
	function catamaran_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'catamaran_filter_responsive_sizes', catamaran_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'catamaran_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			catamaran_fpc( catamaran_get_file_dir( $to ), apply_filters( 'catamaran_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'catamaran_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function catamaran_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'catamaran_add_protocol' ) ) {
	/**
	 ...
	 */
	function catamaran_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? catamaran_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'catamaran_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function catamaran_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'catamaran_add_to_url' ) ) {
	/**
	 ...
	 */
	function catamaran_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = catamaran_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'catamaran_is_url' ) ) {
	/**
	 ...
	 */
	function catamaran_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'catamaran_is_external_url' ) ) {
	/**
	 ...
	 */
	function catamaran_is_external_url( $url ) {
		...
		return catamaran_is_url( $url ) && strpos( $url, catamaran_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}

if ( ! function_exists( 'catamaran_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function catamaran_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => catamaran_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => CATAMARAN_CHILD_DIR,
				'url' => catamaran_remove_protocol_from_url( CATAMARAN_CHILD_URL )
				),
			'theme' => array(
				'dir' => CATAMARAN_THEME_DIR,
				'url' => catamaran_remove_protocol_from_url( CATAMARAN_THEME_URL )
				)
			);
