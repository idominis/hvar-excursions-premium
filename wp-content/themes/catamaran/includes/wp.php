...
if ( ! function_exists( 'catamaran_wp_theme_setup' ) ) {

	add_action( 'after_setup_theme', 'catamaran_wp_theme_setup' );
	 ...
	 */
	function catamaran_wp_theme_setup() {

		// Remove macros from title
		add_filter( 'wp_title', 'catamaran_remove_macros_from_site_title' );
		add_filter( 'wp_title_parts', 'catamaran_remove_macros_from_site_title' );
		add_filter( 'document_title_parts', 'catamaran_remove_macros_from_site_title' );

		// Breadcrumbs link 'All posts'
		add_filter( 'post_type_archive_link', 'catamaran_get_template_page_link', 10, 2 );

		// Adds a link to the 'Privacy Policy' page to the Privacy text
		// (uncomment a next line if you need it)
		// add_filter( 'catamaran_filter_privacy_text', 'catamaran_add_privacy_page_link' );
	}
}


if ( ! function_exists( 'catamaran_is_preview' ) ) {
	/**
	 ...
	 */
	function catamaran_is_preview( $builder = 'any' ) {
		return ( in_array( $builder, array( 'any', 'elm', 'elementor' ) ) && function_exists( 'catamaran_elementor_is_preview' ) && catamaran_elementor_is_preview() )
				||
				( in_array( $builder, array( 'any', 'gb', 'gutenberg' ) ) && function_exists( 'catamaran_gutenberg_is_preview' ) && catamaran_gutenberg_is_preview() );
	}
}


if ( ! function_exists( 'catamaran_get_wp_template_hooks' ) ) {
	/**
	 ...
	 */
	function catamaran_get_wp_template_hooks() {
		return apply_filters( 'catamaran_filter_wp_template_hooks', array(
					'404_template',
					'archive_template',
					'attachment_template',
					'author_template',
...
if ( ! function_exists( 'catamaran_detect_blog_mode' ) ) {
	/**
	 ...
	 * 
	 * For example:
	 *   $pos = catamaran_get_theme_option( 'sidebar_position' );
	 * 
	 ...
	 */
	function catamaran_detect_blog_mode() {
		if ( is_front_page() && ! is_home() ) {
			...
			$mode = '404';
		} elseif ( catamaran_is_single() ) {
			$mode = 'post';
		} elseif ( is_page() && ! catamaran_storage_isset( 'blog_archive' ) ) {
			$mode = 'page';
		} elseif ( is_category() ) {
			$mode = 'category';
		} elseif ( is_tag() ) {
		...
		return apply_filters( 'catamaran_filter_detect_blog_mode', $mode );
	}
}

if ( ! function_exists( 'catamaran_is_blog_mode_custom' ) ) {
	/**
	 ...
	 */
	function catamaran_is_blog_mode_custom() {
		return ! in_array( catamaran_storage_get( 'blog_mode' ), apply_filters( 'catamaran_filter_blog_mode_standard', array(
...
if ( ! function_exists( 'catamaran_get_current_mode_image' ) ) {
	/**
	 ...
	 */
	function catamaran_get_current_mode_image( $default = '' ) {
		if ( is_category() || is_tax() ) {
			$img = catamaran_get_term_image();
			if ( '' != $img ) {
				$default = $img;
			}
		} elseif ( catamaran_is_singular() || catamaran_storage_isset( 'blog_archive' ) ) {
			if ( is_home() ) {
		...
		return catamaran_clear_thumb_size( $default );
	}
}

if ( ! function_exists( 'catamaran_blog_archive_start' ) ) {
	/**
	 ...
	 */
	function catamaran_blog_archive_start() {
		$main_post = catamaran_storage_get( 'blog_archive_template_post' );
		if ( is_object( $main_post ) ) {
			...
			$catamaran_content            = '';
			$catamaran_blog_archive_mask  = '%%CONTENT%%';
			$catamaran_blog_archive_subst = sprintf( '<div class="blog_archive">%s</div>', $catamaran_blog_archive_mask );
			$catamaran_content            = apply_filters( 'the_content', get_the_content() );
			...
			if ( '' != $catamaran_content ) {
				$catamaran_pos = stripos( $catamaran_content, $catamaran_blog_archive_mask );
				if ( false !== $catamaran_pos ) {
					$catamaran_content = preg_replace( '/(\<p\>\s*)?' . $catamaran_blog_archive_mask . '(\s*\<\/p\>)/i', $catamaran_blog_archive_subst, $catamaran_content );
				} else {
					$catamaran_content .= $catamaran_blog_archive_subst;
				}
				$catamaran_content = explode( $catamaran_blog_archive_mask, $catamaran_content );
				// Display first part
				catamaran_show_layout( apply_filters( 'catamaran_filter_blog_archive_start', $catamaran_content[0] ) );
				// And store second part
				catamaran_storage_set( 'blog_archive_end', $catamaran_content[1] );
			}
...
if ( ! function_exists( 'catamaran_blog_archive_end' ) ) {
	/**
	 * Display the end blog archive template.
	 * 
	 * Output a second part (stored in the previous call of catamaran_blog_archive_start).
	 */
	function catamaran_blog_archive_end() {
		$html = catamaran_storage_get( 'blog_archive_end' );
		if ( '' != $html ) {
			// Display second part of template content
			catamaran_show_layout( apply_filters( 'catamaran_filter_blog_archive_end', $html ) );
		}
	}
}

if ( ! function_exists( 'catamaran_blog_archive_get_template' ) ) {
	/**
	 ...
	 */
	function catamaran_blog_archive_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = catamaran_get_theme_option( 'blog_style' );
		}
		$parts   = explode( '_', $blog_style );
		$archive = 'index';
		if ( catamaran_storage_isset( 'blog_styles', $parts[0], 'archive' ) ) {
			$archive = catamaran_storage_get_array( 'blog_styles', $parts[0], 'archive' );
		}
		return apply_filters( 'catamaran_filter_blog_archive_template', $archive, $blog_style );
	}
}

if ( ! function_exists( 'catamaran_blog_item_get_template' ) ) {
	/**
	 ...
	 *                           or an empty string if not detected.
	 */
	function catamaran_blog_item_get_template( $blog_style = '' ) {
		if ( empty( $blog_style ) ) {
			$blog_style = catamaran_get_theme_option( 'blog_style' );
		}
		$blog_style = str_replace( array( '.', '/', '\\' ), '', catamaran_esc( $blog_style ) );
		$parts = explode( '_', $blog_style );
		$item  = '';
		if ( strpos( $parts[0], 'blog-custom-' ) === 0 ) {
			$item = 'templates/content-custom';
		} elseif ( catamaran_storage_isset( 'blog_styles', $parts[0], 'item' ) ) {
			$item = catamaran_storage_get_array( 'blog_styles', $parts[0], 'item' );
		} else {
...
if ( ! function_exists('catamaran_get_the_ID') ) {
	/**
	 * Return current post ID
	 * 
	 * @return int  Current post ID
	 */
	function catamaran_get_the_ID() {
		global $wp_query;
		return in_the_loop() 
					? get_the_ID() 
					: ( ! empty( $wp_query->post->ID )
						? $wp_query->post->ID
						: ( catamaran_is_singular() && ! empty( $wp_query->queried_object->ID )
...
if ( ! function_exists( 'catamaran_get_post_id' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_id( $args = array() ) {
		$args  = array_merge(
			array(
				'posts_per_page' => 1,
			), $args
...
if ( ! function_exists( 'catamaran_get_post_content' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_content( $apply_filters = false ) {
		global $post;
...
if ( ! function_exists( 'catamaran_filter_post_content' ) ) {
	/**
	 ...
	 */
	function catamaran_filter_post_content( $content ) {
		$content = apply_filters( 'catamaran_filter_post_content', $content );
		global $wp_embed;
...
if ( ! function_exists( 'catamaran_get_template_page_id' ) ) {
	/**
	 ...
	 */
	function catamaran_get_template_page_id( $args = array() ) {
		$args   = array_merge(
			array(
				'template'   => 'blog.php',
				'post_type'  => 'post',
		...
		if ( ! empty( $args['post_type'] ) ) {
			$q_args['meta_query'][] = array(
				'key'     => 'catamaran_options_post_type',
		...
		if ( '' !== $args['parent_cat'] ) {
			$q_args['meta_query'][] = array(
				'key'     => 'catamaran_options_parent_cat',
				'value'   => $args['parent_cat'] > 0 ? $args['parent_cat'] : 1,
				'compare' => $args['parent_cat'] > 0 ? '=' : '<',
			);
		}
		return catamaran_get_post_id( $q_args );
	}
}

if ( ! function_exists( 'catamaran_get_template_page_link' ) ) {
	/**
	 ...
	 * 
	 * Handler of the add_filter('post_type_archive_link', 'catamaran_get_template_page_link', 10, 2 );
	 * 
	 ...
	 */
	function catamaran_get_template_page_link( $link = '', $post_type = '' ) {
		if ( ! empty( $post_type ) ) {
			$id = catamaran_get_template_page_id(
				array(
					'post_type'  => $post_type,
					'parent_cat' => 0,
				)
...
if ( ! function_exists( 'catamaran_get_posts_archive_template' ) ) {

	add_filter( 'archive_template', 'catamaran_get_posts_archive_template', 100 );
	 ...
	 */
	function catamaran_get_posts_archive_template( $template ) {
		if ( catamaran_get_theme_option( 'use_blog_archive_pages' ) ) {
			if ( is_post_type_archive() ) {
				$obj = get_queried_object();
				if ( ! empty( $obj->name ) ) {
					$templates = get_option( 'catamaran_blog_archive_templates' );
					if ( ! empty( $templates[ $obj->name ] ) ) {
						$template = catamaran_redirect_to_archive_template( $template, $templates[ $obj->name ] );
					}
				}
			} else {
				$template = catamaran_get_tax_archive_template( $template );
			}
		}
		return $template;
	}	
...
if ( ! function_exists( 'catamaran_get_tax_archive_template' ) ) {

	add_filter( 'category_template', 'catamaran_get_tax_archive_template', 100 );
	add_filter( 'taxonomy_template', 'catamaran_get_tax_archive_template', 100 );
	 ...
	 * @return string          A filtered path to the template
	 */
	function catamaran_get_tax_archive_template( $template ) {
		if ( catamaran_get_theme_option( 'use_blog_archive_pages' ) && ( is_category() || is_tag() || is_tax() ) ) {
			$obj = get_queried_object();
			global $wp_query;
			...
			if ( $pt && $tax && $term ) {
				$pt_tax  = catamaran_get_post_type_taxonomy( $pt );
				if ( $pt_tax == $tax ) {
					$templates = get_option( 'catamaran_blog_archive_templates' );
					if ( ! empty( $templates[ "{$pt}_{$tax}_{$term}" ] ) ) {
						$template = catamaran_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$term}" ] );
					} else {
						$found = false;
						do {
							...
							if ( ! empty( $templates[ "{$pt}_{$tax}_{$parent}" ] ) ) {
								$template = catamaran_redirect_to_archive_template( $template, $templates[ "{$pt}_{$tax}_{$parent}" ], $term );
								$found = true;
								break;
							} else {
						...
						if ( ! $found && ! empty( $templates[ "{$pt}" ] ) ) {
							$template = catamaran_redirect_to_archive_template( $template, $templates[ "{$pt}" ], $term );
						}
					}
				}
			}
...
if ( ! function_exists( 'catamaran_redirect_to_archive_template' ) ) {
	/**
	 ...
	 */
	function catamaran_redirect_to_archive_template( $template, $page_id, $term = false ) {
		// Store page number
		...
		// Load page options
		catamaran_override_theme_options( null, $page_id );
		// Override parent category
		if ( is_object( $term ) ) {
			$term = ! empty( $term->term_id ) ? $term->term_id : 0;
		}
		if ( $term > 0 ) {
			catamaran_storage_set_array( 'options_meta', 'parent_cat', $term );
		}
		return catamaran_get_file_dir( 'blog.php' );
	}
}

if ( ! function_exists( 'catamaran_save_archive_template' ) ) {

	add_action( 'save_post', 'catamaran_save_archive_template', 11 );
	 ...
	 */
	function catamaran_save_archive_template( $post_id ) {
		// verify nonce
		...
		if ( 'page' == $post_type ) {
			...
			if ( 'blog.php' == $page_template ) {
				...
				$tax  = catamaran_get_post_type_taxonomy( $pt );
				$term = isset( $meta['parent_cat'] ) ? $meta['parent_cat'] : 0;
				$templates = catamaran_array_delete_by_value( $templates, $post_id );
				$templates[ $pt . ( $term > 0 ? "_{$tax}_{$term}" : '' ) ] = $post_id;
			} else {
				$templates = catamaran_array_delete_by_value( $templates, $post_id );
			}
			update_option( 'catamaran_blog_archive_templates', $templates );
		}
	}
}

// Delete page from templates
if ( ! function_exists( 'catamaran_delete_archive_template' ) ) {

	add_action( 'delete_post', 'catamaran_delete_archive_template', 11 );
	 ...
	 */
	function catamaran_delete_archive_template( $post_id ) {
		$templates = get_option( 'catamaran_blog_archive_templates' );
		if ( is_array( $templates ) ) {
			$templates = catamaran_array_delete_by_value( $templates, $post_id );
			update_option( 'catamaran_blog_archive_templates', $templates );
...
if ( ! function_exists( 'catamaran_redirect_404' ) ) {
	add_action( 'template_redirect', 'catamaran_redirect_404', 1 );
	/**
	 * Redirect to the 404 page if the page is selected in the theme options.
	 * 
	 * @hooked template_redirect
	 */
	function catamaran_redirect_404() {
		if ( is_404() ) {
			$page_id = catamaran_get_theme_option( 'redirect_404_page', '' );
			if ( ! catamaran_is_off( $page_id ) ) {
				$way = apply_filters( 'catamaran_filter_redirect_404_way', 'show' );	// show | redirect
				if ( $way == 'redirect' ) {
					...
					if ( is_object( $post ) && ! empty( $post->ID ) && $post->ID == $page_id ) {
						...
						catamaran_override_theme_options();
					}
				}
			}
		}
...
if ( ! function_exists( 'catamaran_get_protocol' ) ) {
	/**
	 * Return a current protocol ( http or https ) of the site.
	 * 
	 * @return string A string with a protocol.
	 */
	function catamaran_get_protocol( $suffix = false ) {
		return ( is_ssl() ? 'https' : 'http' ) . ( ! empty( $suffix ) ? ':' : '' );
	}
}

if ( ! function_exists( 'catamaran_get_hash_link' ) ) {
	/**
	 ...
	 */
	function catamaran_get_hash_link( $hash ) {
		if ( 0 !== strpos( $hash, 'http' ) ) {
			if ( '#' != $hash[0] ) {
				$hash = '#' . $hash;
			}
			if ( is_customize_preview() ) {
				$url = catamaran_get_current_url();
				$pos = strpos( $url, '#' );
				if ( false !== $pos ) {
...
if ( ! function_exists( 'catamaran_get_current_url' ) ) {
	/**
	 * Return an URL to the current page.
	 * 
	 * @return string  A current page URL.
	 */
	function catamaran_get_current_url() {
		return add_query_arg( array() );
	}
}

if ( ! function_exists( 'catamaran_check_url' ) ) {
	/**
	 ...
	 */
	function catamaran_check_url( $val ) {
		if ( ! is_array( $val ) ) {
			$val = array( $val );
		}
		$rez = false;
...
if ( ! function_exists( 'catamaran_attachment_url_to_postid' ) ) {
	/**
	 ...
	 */
	function catamaran_attachment_url_to_postid( $url ) {
		static $images = array();
		if ( ! isset( $images[ $url ] ) ) {
			$images[ $url ] = attachment_url_to_postid( catamaran_clear_thumb_size( $url, false ) );
		}
		return $images[ $url ];
	}
}

if ( ! function_exists( 'catamaran_remove_macros_from_site_title' ) ) {
	/**
	 ...
	 */
	function catamaran_remove_macros_from_site_title( $title ) {
		if ( is_array( $title ) ) {
			foreach ( $title as $k => $v ) {
				$title[ $k ] = catamaran_remove_macros( $v );
			}
		} else {
			$title = catamaran_remove_macros( $title );
		}
		return $title;
	}
}

if ( ! function_exists( 'catamaran_get_blog_title' ) ) {
	/**
	 * Return a blog title for the current blog mode.
	 *
	 * @return string  A blog title.
	 */
	function catamaran_get_blog_title() {

		if ( is_front_page() ) {
			$title = esc_html__( 'Home', 'catamaran' );
		} elseif ( is_home() ) {
			$title = get_option( 'page_for_posts' ) == get_queried_object_id()
						? get_the_title( get_queried_object_id() )
						: esc_html__( 'All Posts', 'catamaran' );
		} elseif ( is_author() ) {
			$curauth = ( get_query_var( 'author_name' ) ) ? get_user_by( 'slug', get_query_var( 'author_name' ) ) : get_userdata( get_query_var( 'author' ) );
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Author page: %s', 'catamaran' ), $curauth->display_name );
		} elseif ( is_404() ) {
			$title = esc_html__( 'URL not found', 'catamaran' );
		} elseif ( is_search() ) {
			// Translators: Add the author's name to the title
			$title = sprintf( esc_html__( 'Search: %s', 'catamaran' ), get_search_query() );
		} elseif ( is_day() ) {
			// Translators: Add the queried date to the title
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'catamaran' ), get_the_date() );
		} elseif ( is_month() ) {
			// Translators: Add the queried month to the title
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'catamaran' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			// Translators: Add the queried year to the title
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'catamaran' ), get_the_date( 'Y' ) );
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_tag() ) {
			// Translators: Add the tag's name to the title
			$title = sprintf( esc_html__( 'Tag: %s', 'catamaran' ), single_tag_title( '', false ) );
		} elseif ( is_tax() ) {
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'catamaran' ), get_the_title() );
		} elseif ( catamaran_is_single() || is_page() ) {
			$title = get_the_title();
		} else {
			$title = get_the_title();
		}
		return apply_filters( 'catamaran_filter_get_blog_title', $title );
	}
}

if ( ! function_exists( 'catamaran_get_post_categories' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_categories( $delimiter = ', ', $id = false, $links = true ) {
		return catamaran_get_post_terms( $delimiter, $id, 'category', $links );
	}
}

if ( ! function_exists( 'catamaran_get_post_terms' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_terms( $delimiter = ', ', $id = false, $taxonomy = 'category', $links = true ) {
		$output = '';
		if ( empty( $id ) ) {
			$id = get_the_ID();
		}
		if ( empty( $taxonomy ) ) {
			$taxonomy = catamaran_get_post_type_taxonomy( get_post_type( $id ) );
		}
				...
				$output .= ( ! empty( $output )
							...
							. ( $links
								? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
										...
										. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'catamaran' ), $term->name ) . '"'
										. '>'
								: '<span>'
								)
								. apply_filters( 'catamaran_filter_term_name', $term->name, $term )
							. ( $links
								? '</a>'
								: '</span>'
								);
...
if ( ! function_exists( 'catamaran_get_post_type_taxonomy' ) ) {
	/**
	 ...
	 */
	function catamaran_get_post_type_taxonomy( $post_type = '' ) {
		if ( empty( $post_type ) ) {
		...
		return apply_filters( 'catamaran_filter_post_type_taxonomy', $tax, $post_type );
	}
}

if ( ! function_exists( 'catamaran_get_edited_post_id' ) ) {
	/**
	 ...
	 */
	function catamaran_get_edited_post_id() {
		$id = false;
		if ( is_admin() ) {
			$url = catamaran_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( catamaran_get_value_gp( 'action' ) == 'edit' ) {
					$post_id = catamaran_get_value_gp( 'post' );
					if ( 0 < $post_id ) {
						$id = $post_id;
					}
				}
...
if ( ! function_exists( 'catamaran_get_edited_post_type' ) ) {
	/**
	 ...
	 */
	function catamaran_get_edited_post_type() {
		$pt = '';
		if ( is_admin() ) {
			$url = catamaran_get_current_url();
			if ( strpos( $url, 'post.php' ) !== false ) {
				if ( in_array( catamaran_get_value_gp( 'action' ), array( 'edit', 'elementor' ) ) ) {
					$id = catamaran_get_value_gp( 'post' );
					if ( 0 < $id ) {
				...
				$pt = catamaran_get_value_gp( 'post_type' );
			}
		}
		return $pt;
	}
...
if ( ! function_exists( 'catamaran_is_post_edit' ) ) {
	/**
	 * Detect if a current mode is "Edit post" or "Site editor".
	 *
	 * @return bool Return true if current mode is "Edit post" or "Edit site".
	 */
	function catamaran_is_post_edit() {
		return ( catamaran_check_url( 'post.php' ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'edit')
				||
				catamaran_check_url( 'post-new.php' )
				||
				( catamaran_check_url( '/block-renderer/trx-addons/' ) && ! empty( $_GET['context'] ) && $_GET['context'] == 'edit' )
				||
				( catamaran_check_url( 'admin.php' ) && ! empty( $_GET['page'] ) && $_GET['page'] == 'gutenberg-edit-site' )
				||
				( catamaran_check_url( 'site-editor.php' ) && ! empty( $_GET['postType'] ) )	// || $_GET['postType'] == 'wp_template' ) )
				||
				catamaran_check_url( 'widgets.php' );
	}
}

if ( ! function_exists( 'catamaran_is_singular' ) ) {
	/**
	 ...
	 */
	function catamaran_is_singular( $type = '' ) {
		global $wp_query;
		return apply_filters( 'catamaran_filter_is_singular', ! empty( $wp_query->queried_object->ID ) && is_singular( $type ), $type );
	}
}

if ( ! function_exists( 'catamaran_is_single' ) ) {
	/**
	 * Detect if the current post is a single post.
	 *
	 * @return bool  true if the current mode is a single post.
	 */
	function catamaran_is_single() {
		global $wp_query;
		return apply_filters( 'catamaran_filter_is_single', ! empty( $wp_query->queried_object->ID ) && is_single() );
	}
}

if ( ! function_exists( 'catamaran_add_seo_itemprops' ) ) {
	/**
	 * Add SEO params to the article tag.
	 */
	function catamaran_add_seo_itemprops() {
		if ( catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) ) ) {
			...
			?>" 
			itemtype="<?php echo esc_attr( catamaran_get_protocol( true ) ); ?>//schema.org/<?php echo esc_attr( catamaran_get_markup_schema() ); ?>" 
...
if ( ! function_exists( 'catamaran_add_seo_snippets' ) ) {
	/**
	 * Add SEO meta to the post from the template 'templates/seo.php'
	 */
	function catamaran_add_seo_snippets() {
		if ( catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) ) ) {
...
if ( ! function_exists( 'catamaran_get_nav_menu' ) ) {
	/**
	 ...
	 * 									  or true to use a custom walker from the theme with class 'catamaran_custom_menu_walker'.
	 * 									  Default is false.
	 *
	 * @return string               A menu layout or an error message.
	 */
	function catamaran_get_nav_menu( $location = '', $menu = '', $depth = 0, $custom_walker = false ) {
		static $list = array();
		$slug = $location . '_' . $menu;
		if ( empty( $list[ $slug ] ) ) {
			$list[ $slug ] = esc_html__( 'You are trying to use a menu inserted in himself!', 'catamaran' );
			$args          = array(
								'menu'            => empty( $menu ) || 'default' == $menu || catamaran_is_inherit( $menu ) ? '' : $menu,
								'container'       => '',
								'container_class' => '',
								'container_id'    => '',
								...
								'before'          => '',
								'after'           => '',
								'link_before'     => catamaran_get_theme_setting( 'wrap_menu_items_with_span' ) ? '<span>' : '',
								'link_after'      => catamaran_get_theme_setting( 'wrap_menu_items_with_span' ) ? '</span>' : '',
								'depth'           => $depth,
							);
			if ( ! empty( $location ) ) {
			...
			if ( $custom_walker ) {
				...
				} else if ( $custom_walker === true && class_exists( 'catamaran_custom_menu_walker' ) ) {
					$args['walker'] = new catamaran_custom_menu_walker;
				}
			}
			$list[ $slug ] = wp_nav_menu( apply_filters( 'catamaran_filter_get_nav_menu_args', $args ) );
		}
		return apply_filters( 'catamaran_filter_get_nav_menu', $list[ $slug ], $location, $menu );
	}
}

if ( ! function_exists( 'catamaran_remove_empty_spaces_between_menu_items' ) ) {

    add_action( 'wp_nav_menu', 'catamaran_remove_empty_spaces_between_menu_items', 98, 2 );
	 ...
	 */
	function catamaran_remove_empty_spaces_between_menu_items( $html = '', $args = array() ) {
		return preg_replace(
							array( "/>[\r\n\s]*<li/", "/>[\r\n\s]*<\\/ul>/" ),
							array( "><li", "></ul>" ),
							$html
...
if ( ! function_exists( 'catamaran_remove_empty_menu_items' ) ) {

	add_action( 'wp_nav_menu', 'catamaran_remove_empty_menu_items', 99, 2 );
	 ...
	 * @return string       A clean string.
	 */
	function catamaran_remove_empty_menu_items( $html = '', $args = array() ) {
		return catamaran_get_theme_setting( 'remove_empty_menu_items' )
					? preg_replace(
							"/<li[^>]*>[\r\n\s]*<a[^>]*>[\r\n\s]*(<span>[\r\n\s]*<\\/span>[\r\n\s]*)?<\\/a>[\r\n\s]*<\\/li>/",
							"",
							$html
...
//
if ( ! function_exists( 'catamaran_new_main_query' ) ) {
	/**
	 ...
	 */
	function catamaran_new_main_query( $args ) {
		...
		if ( ! empty( $args['post_type'] ) || ! empty( $args['category'] ) ) {
			$query_args  = catamaran_query_add_posts_and_cats( $query_args, $args['post_ids'], $args['post_type'], $args['category'] );
		}
		if ( $args[ 'page' ] > 1 ) {
...
if ( ! function_exists( 'catamaran_query_add_sort_order' ) ) {
	/**
	 ...
	 */
	function catamaran_query_add_sort_order( $args, $orderby = 'date', $order = 'desc' ) {
		if ( ! empty( $orderby ) && ( empty( $args['orderby'] ) || 'none' != $orderby ) ) {
			$q          = apply_filters( 'catamaran_filter_add_sort_order', array(), $orderby, $order );
		...
		return apply_filters( 'catamaran_filter_add_sort_order_args', $args, $orderby, $order );
	}
}

if ( ! function_exists( 'catamaran_query_add_posts_and_cats' ) ) {
	/**
	 ...
	 */
	function catamaran_query_add_posts_and_cats( $args, $ids = '', $post_type = '', $cat = '', $taxonomy = '' ) {
		if ( ! empty( $ids ) ) {
			...
			if ( ! empty( $cat ) ) {
				...
				if ( empty( $taxonomy ) ) {
					$taxonomy = catamaran_get_post_type_taxonomy( $post_type );
				}
...
if ( ! function_exists( 'catamaran_query_add_filters' ) ) {
	/**
	 ...
	 */
	function catamaran_query_add_filters( $args, $filters = false ) {
		if ( ! empty( $filters ) ) {
			if ( ! is_array( $filters ) ) {
...
if ( ! function_exists( 'catamaran_create_widgets_area' ) ) {
	/**
	 ...
	 */
	function catamaran_create_widgets_area( $name, $add_classes = '' ) {
		$widgets_name = catamaran_get_theme_option( $name, 'hide' );
		if ( ! catamaran_is_off( $widgets_name ) && is_active_sidebar( $widgets_name ) ) {
			catamaran_storage_set( 'current_sidebar', $name );
			ob_start();
			...
			if ( ! empty( $out ) ) {
				...
				$need_columns = strpos( $out, 'columns_wrap' ) === false && apply_filters( 'catamaran_filter_widgets_area_need_columns', 'footer' == $name, $name, $out );
				if ( $need_columns ) {
					$columns = apply_filters( 'catamaran_filter_widgets_area_columns', min( 4, max( 1, catamaran_tags_count( $out, 'aside' ) ) ), $name );
				...
				<div class="<?php echo esc_attr( $name ); ?> <?php echo esc_attr( $name ); ?>_wrap widget_area">
					<?php do_action( 'catamaran_action_before_sidebar_wrap' ); ?>
					<div class="<?php echo esc_attr( $name ); ?>_inner <?php echo esc_attr( $name ); ?>_inner widget_area_inner">
						<?php
						do_action( 'catamaran_action_before_sidebar' );
						catamaran_show_layout(
							$out,
							true == $need_columns ? '<div class="columns_wrap">' : '',
							true == $need_columns ? '</div>' : ''
						);
						do_action( 'catamaran_action_after_sidebar' );
						?>
					</div>
					<?php do_action( 'catamaran_action_after_sidebar_wrap' ); ?>
				</div>
				<?php
			}
		}
...
if ( ! function_exists( 'catamaran_sidebar_present' ) ) {
	/**
	 ...
	 */
	function catamaran_sidebar_present() {
		global $wp_query;
		$sidebar_position = catamaran_get_theme_option( 'sidebar_position' );
		$sidebar_type     = catamaran_get_theme_option( 'sidebar_type' );
		$sidebar_name     = catamaran_get_theme_option( 'sidebar_widgets' );
		return apply_filters(
			'catamaran_filter_sidebar_present',
			! catamaran_is_off( $sidebar_position )
				&& (
						( 'default' == $sidebar_type && ! catamaran_is_off( $sidebar_name ) && is_active_sidebar( $sidebar_name ) )
						||
						( 'custom' == $sidebar_type && catamaran_is_layouts_available() )
					)
				&& ! is_404()
...
if ( ! function_exists( 'catamaran_get_content_width' ) ) {
	/**
	 ...
	 */
	function catamaran_get_content_width() {
		$pg_width = apply_filters( 'catamaran_filter_content_width', (int)catamaran_get_theme_option( 'page_width' ) );
		$sb_width = 0;
		$sb_gap   = 0;
		if ( catamaran_sidebar_present() ) {
			...
			if ( 'default' == $catamaran_sidebar_type ) {
				...
				if ( is_active_sidebar( $catamaran_sidebar_name ) ) {
					$sb_width = (int)catamaran_get_theme_option( 'sidebar_width' );
					$sb_gap   = (int)catamaran_get_theme_option( 'sidebar_gap' );
				}
			} else {
				$sb_width = (int)catamaran_get_theme_option( 'sidebar_width' );
				$sb_gap   = (int)catamaran_get_theme_option( 'sidebar_gap' );
...
if ( ! function_exists( 'catamaran_add_inline_css_class' ) ) {
	/**
	 ...
	 */
	function catamaran_add_inline_css_class( $css, $suffix = '' ) {
		$class_name = catamaran_generate_id( 'catamaran_inline_' );
...
if ( ! function_exists( 'catamaran_add_inline_css' ) ) {
	/**
...
------------------------------------------------------------------------------------- */
if ( ! function_exists( 'catamaran_enqueue_styles' ) ) {
	/**
	 * Enqueue a list of styles
	 *
	 * @param array $list - styles to enqueue
	 */
	function catamaran_enqueue_styles( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = catamaran_get_file_url( $data['src'] );
				if ( $lib_url ) {
					wp_enqueue_style(
						$handle,
						$lib_url,
						...
						! empty( $data['media'] ) ? catamaran_media_for_load_css_responsive( str_replace( '_', '-', $sc ), $data['media'] ) : 'all'
					);
				}
			}
		}
...
if ( ! function_exists( 'catamaran_enqueue_scripts' ) ) {
	/**
	 * Enqueue a list of scripts
	 *
	 * @param array $list - scripts to enqueue
	 */
	function catamaran_enqueue_scripts( $list, $sc ) {
		if ( is_array( $list ) ) {
			foreach( $list as $handle => $data ) {
				$lib_url = catamaran_get_file_url( $data['src'] );
				if ( $lib_url ) {
...
if ( ! function_exists( 'catamaran_enqueue_optimized' ) ) {
	/**
	 ...
	 */
	function catamaran_enqueue_optimized( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && catamaran_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
			...
			if ( ! isset( $args['need'] ) || $args['need'] ) {
				if ( ! empty( $args['css'] ) ) {
					catamaran_enqueue_styles( $args['css'], $sc );
				}
				if ( ! empty( $args['js'] ) ) {
					catamaran_enqueue_scripts( $args['js'], $sc );
				}
...
if ( ! function_exists( 'catamaran_enqueue_optimized_responsive' ) ) {
	/**
	 ...
	 * @param array $args - arguments with styles and scripts to enqueue
	 */
	function catamaran_enqueue_optimized_responsive( $sc, $force, $args ) {
		static $loaded = array();
		if ( empty( $loaded[ $sc ] ) && (
			current_action() == 'wp_enqueue_scripts' && catamaran_need_frontend_scripts( ! empty( $args['slug'] ) ? $args['slug'] :  $sc )
			||
...
if ( ! function_exists( 'catamaran_get_date' ) ) {
	/**
	 ...
	 */
	function catamaran_get_date( $dt = '', $format = '', $disable_date_diff = false ) {
		$diff   = $disable_date_diff ? 0 : intval( catamaran_get_theme_option( 'time_diff_before' ) );
		...
		if ( $diff == 0 || $dt_cur - $dt > $diff * 24 * 3600 ) {
			...
			$dt = sprintf( esc_html__( '%s ago', 'catamaran' ), human_time_diff( $dt, $dt_cur ) );
		}
		return $dt;
	}
}
...
if ( ! function_exists( 'catamaran_lazy_load_off' ) ) {
	/**
	 * Disable a lazy load behaviour for rest images on the current page
	 */
	function catamaran_lazy_load_off() {
		if ( ! catamaran_lazy_load_is_off() ) {
			add_filter( 'wp_lazy_loading_enabled', 'catamaran_lazy_load_disabled' );
		}
	}
}

if ( ! function_exists( 'catamaran_lazy_load_disabled' ) ) {
	/**
	 ...
	 */
	function catamaran_lazy_load_disabled() {
		return false;
	}
}

if ( ! function_exists( 'catamaran_lazy_load_on' ) ) {
	/**
	 * Enable a lazy load behaviour for rest images on the current page
	 */
	function catamaran_lazy_load_on() {
		if ( catamaran_lazy_load_is_off() ) {
			remove_action( 'wp_lazy_loading_enabled', 'catamaran_lazy_load_disabled' );	// Is equal to rf
		}
	}
}

if ( ! function_exists( 'catamaran_lazy_load_is_off' ) ) {
	/**
	 * Return a current state of the lazy load behaviour
	 *
	 * @return bool  A current lazy load state
	 */
	function catamaran_lazy_load_is_off() {
		return has_filter( 'wp_lazy_loading_enabled', 'catamaran_lazy_load_disabled' );
	}
}
...
if ( ! function_exists( 'catamaran_get_markup_schema' ) ) {
	/**
	 * Return a structured data markup schema for the current page.
	 *
	 * @return string  A schema of the current page.
	 */
	function catamaran_get_markup_schema() {
		if ( catamaran_is_single() ) {                                        // Is single post
			$type = 'Article';
...
if ( ! function_exists( 'catamaran_get_privacy_text' ) ) {
	/**
	 * Return a text for the Privacy Policy checkbox.
	 *
	 * @return string  A label text.
	 */
	function catamaran_get_privacy_text() {
		return apply_filters( 'catamaran_filter_privacy_text', wp_kses( trim( catamaran_get_theme_option( 'privacy_text' ) ), 'catamaran_kses_content' ) );
	}
}

if ( ! function_exists( 'catamaran_add_privacy_page_link' ) ) {
	/**
	 * Adds the Privacy Policy page link to the Privacy text
	 * 
	 * @hooked filter catamaran_filter_privacy_text
	 * 
	 * @param string $text - the text to modify
	 *
	 * @return string  A label text.
	 */
	function catamaran_add_privacy_page_link( $text ) {
		if ( ! empty( $text ) ) {
			...
			if ( ! empty( $page ) ) {
				$text .=  ( ! in_array( substr( $text, -1 ), array( '.', '!' ) ) ? '.' : '' )
							...
							. sprintf(
								__( 'For further details on handling user data, see our %s.', 'catamaran' ),
								'<a href="' . esc_url( get_permalink( $page ) ) . '"' . catamaran_external_links_target() . '>' . __( 'Privacy Policy', 'catamaran' ) . '</a>'
							);
			}
		}
		return $text;
...
if ( ! function_exists( 'catamaran_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'catamaran_kses_allowed_html', 10, 2 );
	/**
	 * Return a theme-specific tags list with allowed attributes for using in the wp_kses() calls
	 * ( if $context is equal to 'catamaran_kses_content' or 'trx_addons_kses_content' ).
	 *
	 ...
	 */
	function catamaran_kses_allowed_html( $tags, $context ) {
		if ( in_array( $context, array( 'catamaran_kses_content', 'trx_addons_kses_content' ) ) ) {
			$tags = array( 
...
if ( ! function_exists( 'catamaran_verify_nonce' ) ) {
	/**
	 ...
	 */
	function catamaran_verify_nonce( $nonce = 'nonce', $mask = '', $die = true ) {
		$rez = true;
		if ( empty( $mask ) ) {
			$mask = admin_url('admin-ajax.php');
		}
		if ( ! wp_verify_nonce( catamaran_get_value_gp( $nonce ), $mask ) ) {
			$rez = false;
			if ( $die ) {
				catamaran_forbidden();
			}
...
if ( ! function_exists( 'catamaran_exit' ) ) {
	/**
	 ...
	 */
	function catamaran_exit( $message = '', $title = '', $code = 200 ) {
		wp_die( $message, $title, array( 'response' => $code, 'exit' => empty( $message ) && empty( $title ) ) );
	}
}

if ( ! function_exists( 'catamaran_forbidden' ) ) {
	/**
	 ...
	 * @param string $title    Optional. Title to output while exit.
	 */
	function catamaran_forbidden( $message = '', $title = '' ) {
		catamaran_exit( $message, $title, 403 );
	}
}

if ( ! function_exists( 'catamaran_ajax_response' ) ) {
	/**
	 * Send an ajax response and exit.
	 *
	 * @param mixed $response  A response to send to the client.
	 */
	function catamaran_ajax_response( $response ) {
...
if ( ! function_exists( 'catamaran_get_upgrade_url' ) ) {
	/**
	 ...
	 */
	function catamaran_get_upgrade_url( $params = array() ) {
		$theme_folder = get_template();
		$theme_slug   = apply_filters( 'catamaran_filter_original_theme_slug', $theme_folder );
		$default      = array(
							'action'     => '',
		...
		if ( ! empty( $params['action'] ) && ! in_array( $params['action'], array( 'info_skins' ) ) ) {
			$default['key']        = '';
			$default['src']        = catamaran_get_theme_pro_key();
			$default['theme_name'] = wp_get_theme( $theme_folder )->get( 'Name' );
			$default['domain']     = catamaran_remove_protocol_from_url( home_url(), true );
		...
		return catamaran_add_to_url( trailingslashit( catamaran_storage_get( 'theme_upgrade_url' ) ) . 'upgrade.php', $params );
	}
}

if ( ! function_exists( 'catamaran_get_upgrade_data' ) ) {
	/**
	 ...
	 */
	function catamaran_get_upgrade_data( $params = array() ) {
		$url = catamaran_get_upgrade_url( $params );
		$result = function_exists( 'trx_addons_fgc' ) ? trx_addons_fgc( $url ) : catamaran_fgc( $url );
		if ( is_serialized( $result ) ) {
			try {
				$result = catamaran_unserialize( $result );
			} catch ( Exception $e ) {
			}
		}
		if ( ! isset( $result['error'] ) || ! isset( $result['data'] ) ) {
			$result = array(
				'error' => esc_html__( 'Unrecognized server answer!', 'catamaran' ),
				'data'  => ''
			);
		}
		return $result;
...
if ( ! function_exists( 'catamaran_allow_upload_archives' ) ) {
	/**
	 * Allow WordPress to upload archives before get an upgrade package from the upgrade server.
	 */
	function catamaran_allow_upload_archives() {
...
if ( ! function_exists( 'catamaran_disallow_upload_archives' ) ) {
	/**
	 * Disallow WordPress to upload archives after the upgrade package received.
	 */
	function catamaran_disallow_upload_archives() {
