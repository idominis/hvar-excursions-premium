...
( function( $ ) {
	...
	var $style = $( '#catamaran-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length === 0 ) {
		var style_container = '<' + 'style type="text/css" id="catamaran-customizer-inline-css" />',
			$theme_css      = $( '#catamaran-custom-inline-css' );
		if ($theme_css.length === 0) {
			$theme_css = $( '#catamaran-custom-css' );
		}
		if ($theme_css.length === 0) {
		...
		$style = $( '#catamaran-customizer-inline-css' );
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf( 'body_style' ) === 0) {
				...
				$( '.sc_layouts_logo .logo_text' ).html( catamaran_prepare_macros( val ) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( catamaran_prepare_macros( val ) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( catamaran_prepare_macros( val ) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color_type == 'none') {
						...
						color = typeof catamaran_color_schemes[scheme] != 'undefined' ? catamaran_color_schemes[scheme]['colors']['bg_color'] : '';
					}
					$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : catamaran_hex2rgba( color, mask ) ) );
					...
					.height( 'auto' )
					.toggleClass( 'catamaran-full-height sc_layouts_flex sc_layouts_columns_middle', val );
					$( window ).trigger( 'resize' );
				}
			}
		} );
		...
		function add_color_scheme_helpers() {
			$( '[class*="scheme_"]:not(html)' ).each( function() {
				var $self = $( this ),
					...
					scheme = catamaran_get_class_by_prefix( classes, 'scheme_' ).replace( 'scheme_', '' );
				$self.append( '<div class="catamaran_customizer_scheme_helper">'
									+ '<span class="catamaran_customizer_scheme_helper_name">'
										+ tag
										+ ( id
											? '#' + id
											: ( classes
		...
		function remove_color_scheme_helpers() {
			$( '.catamaran_customizer_scheme_helper' ).remove();
		}
