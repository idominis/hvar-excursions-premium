<?php
if ( ! function_exists( 'catamaran_add_css_vars' ) ) {
	add_filter( 'catamaran_filter_get_css', 'catamaran_add_css_vars', 1, 2 );
	/**
	 * Add a theme-specific fonts, vars and colors to the custom CSS
	 * 
	 * @hooked catamaran_filter_get_css, 1
	 * 
	 ...
	 */
	function catamaran_add_css_vars( $css, $args ) {
						...
						if ( is_array( $font ) ) {
							foreach ( $font as $css_prop => $css_value ) {
								...
								if ( empty( $suffix ) || ! empty( $font["{$css_prop}{$suffix}"] ) ) {
									...
									if ( ( ! empty( $font["{$param}{$suffix}"] ) && ! catamaran_is_inherit( $font["{$param}{$suffix}"] ) ) || strpos( $css_prop, 'color' ) === false ) {
										$tmp .= "--theme-font-{$tag}_{$css_name}: " . ( ! empty( $font["{$param}{$suffix}"] ) && ! catamaran_is_inherit( $font["{$param}{$suffix}"] )
																		? ( in_array( $css_prop, array( 'font-size', 'letter-spacing', 'margin-top', 'margin-bottom', 'border-width', 'border-radius' ) )
																			? catamaran_prepare_css_value( $font["{$param}{$suffix}"] )
																			: $font["{$param}{$suffix}"]
																			)
																		: 'inherit'
																	) . ";\n";
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $CATAMARAN_STORAGE['options'] ) && count( $CATAMARAN_STORAGE['options'] ) > 0 ) {
				$breakpoints = catamaran_get_theme_breakpoints();
				foreach ( $breakpoints as $bp => $bpv ) {
				...
				foreach ( $CATAMARAN_STORAGE['options'] as $option => $value ) {
					if ( ! empty( $value['css'] ) ) {
						...
						foreach ( $breakpoints as $bp => $bpv ) {
							...
							if ( $css_value !== '' ) {
								$css_value = catamaran_prepare_css_value( $css_value );
		...
		if ( isset( $css['vars'] ) && isset( $args['vars'] ) ) {
			...
			if ( is_array( $vars ) && count( $vars ) > 0 ) {
				...
				if ( isset( $vars['sidebar_gap'] ) ) {
					$vars['sidebar_gap_width'] = catamaran_prepare_css_value( $vars['sidebar_gap'] );
				}
				// Remove calculated values from css variables
				$exclude = apply_filters( 'catamaran_filter_exclude_theme_vars', array( 'sidebar_gap' ) );	//Old case: array( 'sidebar_width', 'sidebar_gap' )
