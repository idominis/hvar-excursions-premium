...
if ( is_array( $catamaran_template_args ) ) {
	...
	if ( ! empty( $catamaran_template_args['slider'] ) ) {
		...
		?>
		<div class="<?php echo esc_attr( $catamaran_columns_class ); ?>">
		<?php
	}
} else {
...
<article id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>"
	<?php
	post_class( 'post_item post_item_container post_layout_excerpt post_format_' . esc_attr( $catamaran_post_format ) );
...
>
	...
	catamaran_show_post_featured( apply_filters( 'catamaran_filter_args_featured',
		array(
			...
			'thumb_size' => ! empty( $catamaran_template_args['thumb_size'] )
							...
							: catamaran_get_thumb_size( strpos( catamaran_get_theme_option( 'body_style' ), 'full' ) !== false
								? 'full'
								: ( $catamaran_expanded 
									? 'huge' 
									: 'big' 
	...
	if ( $catamaran_show_title ) {
		...
		<div class="post_header entry-header">
			<?php
			// Post title
			if ( apply_filters( 'catamaran_filter_show_blog_title', true, 'excerpt' ) ) {
	...
	// Post content
	if ( apply_filters( 'catamaran_filter_show_blog_excerpt', empty( $catamaran_template_args['hide_excerpt'] ) && catamaran_get_theme_option( 'excerpt_length' ) > 0, 'excerpt' ) ) {
		?>
		<div class="post_content entry-content">
			<?php

			// Post meta
			if ( apply_filters( 'catamaran_filter_show_blog_meta', $catamaran_show_meta, $catamaran_components, 'excerpt' ) ) {
			...
			if ( catamaran_get_theme_option( 'blog_content' ) == 'fullpost' ) {
				// Post content area
				?>
			...
			// More button
			if ( apply_filters( 'catamaran_filter_show_blog_readmore',  ! isset( $catamaran_template_args['more_button'] ) || ! empty( $catamaran_template_args['more_button'] ), 'excerpt' ) ) {
				if ( empty( $catamaran_template_args['no_links'] ) ) {
					do_action( 'catamaran_action_before_post_readmore' );
					if ( catamaran_get_theme_option( 'blog_content' ) != 'fullpost' ) {
						catamaran_show_post_more_link( $catamaran_template_args, '<p>', '</p>' );
					} else {
						catamaran_show_post_comments_link( $catamaran_template_args, '<p>', '</p>' );
					}
