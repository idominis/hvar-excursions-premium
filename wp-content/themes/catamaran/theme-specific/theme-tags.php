...
if ( ! function_exists( 'catamaran_need_page_title' ) ) {
	/**
	 * Check if the current page is need a title.
     *
     * Trigger a filter 'catamaran_filter_need_page_title' to allow other modules to modify a result.
	 *
	 * @return bool  Return true if the current page is need a title.
	 */
	function catamaran_need_page_title() {
		return ! is_front_page() && apply_filters( 'catamaran_filter_need_page_title', true );
	}
}

if ( ! function_exists( 'catamaran_show_layout' ) ) {
	/**
     ...
     */
	function catamaran_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'catamaran_get_logo_image' ) ) {
	/**
	 ...
	 */
	function catamaran_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = catamaran_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = catamaran_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ), '' );
		}
		$logo_retina = catamaran_is_on( catamaran_get_theme_option( 'logo_retina_enabled' ) )
						? catamaran_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina', '' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? catamaran_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? catamaran_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

if ( ! function_exists( 'catamaran_get_header_video' ) ) {
	/**
	 * Return a video URL to display in the header (if set).
	 *
	 * Trigger a filter 'catamaran_header_video_enable' to detect if a video allowed for the current page.
	 *
	 * @return string  URL of the video for a header
	 */
	function catamaran_get_header_video() {
		$video = '';
		if ( apply_filters( 'catamaran_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( catamaran_check_theme_option( 'header_video' ) ) {
				$video = catamaran_get_theme_option( 'header_video' );
...
if ( ! function_exists( 'catamaran_show_post_featured_image' ) ) {
	/**
	 ...
	 */
	function catamaran_show_post_featured_image( $args = array() ) {
		...
		if ( ( ! catamaran_sc_layouts_showed( 'featured' ) && strpos( catamaran_get_post_content(), '[trx_widget_banner]' ) === false ) || in_array( $post_format, array( 'audio', 'video', 'gallery' ) ) ) {
			do_action( 'catamaran_action_before_post_featured' );
			catamaran_show_post_featured( $args );
			do_action( 'catamaran_action_after_post_featured' );
		} elseif ( catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) ) && has_post_thumbnail() ) {
			?>
			<meta itemprop="image" itemtype="<?php echo esc_attr( catamaran_get_protocol( true ) ); ?>//schema.org/ImageObject" content="<?php echo esc_url( wp_get_attachment_url( get_post_thumbnail_id() ) ); ?>">
			<?php
		}
	}
}

if ( ! function_exists( 'catamaran_show_post_title_and_meta' ) ) {
	/**
	 ...
	 */
	function catamaran_show_post_title_and_meta( $args = array() ) {
		...
		// Title and post meta
		if ( ( ! catamaran_sc_layouts_showed( 'title' ) || ! catamaran_sc_layouts_showed( 'postmeta' ) ) ) {
			...
			<div class="post_header post_header_single entry-header">
				...
				do_action( 'catamaran_action_post_header_start' );
				
				// Post meta
				$meta_components = catamaran_array_get_keys_by_value( catamaran_get_theme_option( 'meta_parts' ) );
				$show_categories = in_array( 'categories', $meta_components );
				$meta_components = catamaran_array_delete_by_value( $meta_components, 'categories' );
				$meta_components = catamaran_array_delete_by_value( $meta_components, 'likes' );
				$share_position  = catamaran_array_get_keys_by_value( catamaran_get_theme_option( 'share_position', 'top=0|left=0|bottom=1' ) );
				if ( ! in_array( 'top', $share_position ) ) {
					$meta_components = catamaran_array_delete_by_value( $meta_components, 'share' );
				}
				$seo = catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) );
				if ( ! empty( $args['show_title'] ) ) {
					if ( $show_categories && ! catamaran_sc_layouts_showed( 'postmeta' ) && catamaran_is_on( catamaran_get_theme_option( 'show_post_meta' ) ) ) {
					...
					// Post title
					if ( ! catamaran_sc_layouts_showed( 'title' ) ) {
						the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
					}
					// Post subtitle
					$post_subtitle = catamaran_get_theme_option( 'post_subtitle', '' );
					if ( ! empty( $post_subtitle ) ) {
						?>
						<div class="post_subtitle">
							<?php catamaran_show_layout( $post_subtitle ); ?>
						</div>
						<?php
					}
				}
				// Post meta
				if ( ! empty( $args['show_meta'] ) && ! catamaran_sc_layouts_showed( 'postmeta' ) && catamaran_is_on( catamaran_get_theme_option( 'show_post_meta' ) ) ) {
					if ( ! empty( $args['split_meta_by'] ) ) {
						?><div class="post_meta_other"><?php
							$meta_components = catamaran_get_theme_option( 'meta_parts' );
							...
							$part1 = array_keys( catamaran_array_slice( $meta_components, '', $args['split_meta_by'] ), 1 );
							$part2 = array_keys( catamaran_array_slice( $meta_components, '+' . $args['split_meta_by'], '' ), 1 );
							if ( ! in_array( 'top', $share_position ) ) {
								$part2 = catamaran_array_delete_by_value( $part2, 'share' );
							}
							catamaran_show_post_meta(
								apply_filters(
									'catamaran_filter_post_meta_args',
							...
							catamaran_show_post_meta(
								apply_filters(
									...
									array_merge(
										...
										$args,
										array(
											'show_labels' => ! empty( $args['show_labels'] ) || ! in_array( 'share', $part2 ) || ! catamaran_exists_trx_addons()
										)
									),
									'single',
									1
				...
				do_action( 'catamaran_action_post_header_end' );
			...
			ob_end_clean();
			if ( strpos( $catamaran_post_header, 'post_subtitle' ) !== false
...
if ( ! function_exists( 'catamaran_show_post_content' ) ) {
	/**
	 ...
	 */
	function catamaran_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$simple = true;
		...
		if ( has_excerpt() ) {
			...
			do_action( 'catamaran_action_before_full_post_content' );
			catamaran_show_layout( catamaran_filter_post_content( get_the_content('') ) );
			do_action( 'catamaran_action_after_full_post_content' );
			$simple = false;
			...
			$quote = catamaran_get_tag( catamaran_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				catamaran_show_layout( wpautop( $quote ) );
				$simple = false;
			} else {
				catamaran_show_layout( catamaran_filter_post_content( get_the_content() ) );
			}
			...
			if ( $simple ) {
				$len = isset( $args['hide_excerpt'] ) && (int) $args['hide_excerpt'] > 0
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: catamaran_get_theme_option( 'excerpt_length' )
								);
				$output = catamaran_excerpt( $output, $len );
			}
		}
		if ( trim( str_replace( "&nbsp;", '', $output ) ) != '' ) {
			catamaran_show_layout( trim( $output ), $otag, $ctag);
		}
	}
}

if ( ! function_exists( 'catamaran_show_post_more_link' ) ) {
	/**
	 ...
	 * @param string $ctag  Optional. A close tag to wrap a result. Default is empty string.
	 */
	function catamaran_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
			...
			catamaran_show_layout(
				'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
					. ( ! empty( $args['more_text'] )
							...
							: esc_html__( 'Read more', 'catamaran' )
							)
				. '</a>',
				$otag,
				$ctag
...
if ( ! function_exists( 'catamaran_show_post_comments_link' ) ) {
	/**
	 ...
	 * @param string $ctag  Optional. A close tag to wrap a result. Default is empty string.
	 */
	function catamaran_show_post_comments_link( $args = array(), $otag='', $ctag='' ) {
		...
		catamaran_show_layout(
			'<a class="more-link comments-link" href="' . esc_url( get_comments_link() ) . '">'
				. ( ! empty( $args['comments_text'] )
						...
						: ( $total == 0
							? esc_html__( 'Leave a comment', 'catamaran' )
							: ( $total == 1 ? esc_html__( 'View comment', 'catamaran' ) : esc_html__( 'View comments', 'catamaran' ) )
							)
						)
			. '</a>',
			$otag,
...
if ( ! function_exists( 'catamaran_show_post_pagination' ) ) {
	/**
	 * Show the single post pagination links.
	 *
	 * Trigger actions: 'catamaran_action_before_post_pagination' and 'catamaran_action_after_post_pagination'
	 * to allow other modules wrap the result.
	 */
	function catamaran_show_post_pagination() {
		do_action( 'catamaran_action_before_post_pagination' );
		wp_link_pages(
			array(
				'before'      => '<div class="page_links"><span class="page_links_title">' . esc_html__( 'Pages:', 'catamaran' ) . '</span>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'catamaran' ) . ' </span>%',
				'separator'   => '<span class="screen-reader-text">, </span>',
			)
		);
		do_action( 'catamaran_action_after_post_pagination' );
	}
}

if ( ! function_exists( 'catamaran_show_post_footer' ) ) {
	/**
     ...
     */
	function catamaran_show_post_footer( $components = 'pages,tags,likes,share,prev_next,author' ) {
		...
		foreach( $components as $comp ) {
			...
			if ( 'tags' == $comp ) {
				...
				ob_start();
				the_tags( '', '', '' );
				$catamaran_meta_output = ob_get_contents();
				ob_end_clean();
				if ( ! empty( $catamaran_meta_output ) ) {
					do_action( 'catamaran_action_before_post_tags' );
					catamaran_show_layout( $catamaran_meta_output, '<div class="post_tags_single"><span class="post_meta_label">' . esc_html__( 'Tags:', 'catamaran' ) . '</span> ', '</div>' );
					do_action( 'catamaran_action_after_post_tags' );
				}

			} else if ( 'likes' == $comp ) {

				// Emotions
				if ( catamaran_exists_trx_addons() && function_exists( 'trx_addons_get_post_reactions' ) && trx_addons_is_on( trx_addons_get_option( 'emotions_allowed', 0 ) ) ) {
				...
				if ( in_array( 'likes', $components ) && in_array( 'likes', $meta_components )
						&&
						( ! function_exists( 'trx_addons_get_option' ) || trx_addons_is_off( trx_addons_get_option( 'emotions_allowed', 0 ) ) || ! apply_filters( 'trx_addons_filter_show_post_reactions', catamaran_is_single() && ! is_attachment() ) )
				) {
					$meta_footer[] = 'likes';
				}
				...
				if ( count( $meta_footer) > 0 ) {
					...
					if ( ! empty( $catamaran_meta_output ) ) {
						...
						catamaran_show_layout( $catamaran_meta_output );
				...
				// Author bio
				if ( catamaran_get_theme_option( 'show_author_info' ) == 1
					&& ! is_attachment()
					&& ( apply_filters( 'catamaran_filter_show_author_without_bio', true ) || get_the_author_meta( 'description' ) )
					&& ( 'scroll' != $catamaran_posts_navigation || (int)catamaran_get_theme_option( 'posts_navigation_scroll_hide_author', 0 ) == 0 )
					&& ( ! $full_post_loading || (int)catamaran_get_theme_option( 'open_full_post_hide_author', 1 ) == 0 )
				) {
					do_action( 'catamaran_action_before_post_author' );
					get_template_part( apply_filters( 'catamaran_filter_get_template_part', 'templates/author-bio' ) );
					do_action( 'catamaran_action_after_post_author' );
				}

			} else if ( 'prev_next' == $comp ) {

				// Previous/next post navigation.
				if ( 'links' == $catamaran_posts_navigation && ! $full_post_loading ) {
					do_action( 'catamaran_action_before_post_navigation' );
					?>
					<div class="nav-links-single<?php
						if ( catamaran_get_theme_setting( 'thumbs_in_navigation' ) ) {
					...
					?>">
						...
						the_post_navigation( apply_filters( 'catamaran_filter_post_navigation_args', array(
							'next_text' => ( catamaran_get_theme_setting( 'thumbs_in_navigation' ) ? '<span class="nav-arrow"></span>' : '' )
								. '<span class="nav-arrow-label">' . esc_html__( 'Next', 'catamaran' ) . '</span> '
								. '<h6 class="post-title">%title</h6>'
								. '<span class="post_date">%date</span>',
							'prev_text' => ( catamaran_get_theme_setting( 'thumbs_in_navigation' ) ? '<span class="nav-arrow"></span>' : '' )
								. '<span class="nav-arrow-label">' . esc_html__( 'Previous', 'catamaran' ) . '</span> '
								. '<h6 class="post-title">%title</h6>'
								. '<span class="post_date">%date</span>',
						), 'post_footer' ) );
						?>
					...
					<?php
					do_action( 'catamaran_action_after_post_navigation' );
				}
...
if ( ! function_exists( 'catamaran_show_post_meta' ) ) {
	/**
	 ...
	 */
	function catamaran_show_post_meta( $args = array() ) {
		if ( catamaran_is_single() && catamaran_is_off( catamaran_get_theme_option( 'show_post_meta' ) ) ) {
			return ' ';  // Need at least one space!
		}
		$args = array_merge(
			apply_filters( 'catamaran_filter_post_meta_args_default', array(
		...
		if ( catamaran_storage_isset( 'options', 'meta_parts', 'val' ) ) {
			$parts = explode( '|', catamaran_get_theme_option( 'meta_parts' ) );
			$list_new = array();
			foreach( $parts as $part ) {
				$part = explode( '=', $part );
				if ( in_array( $part[0], $components ) ) {
					$list_new[] = $part[0];
					$components = catamaran_array_delete_by_value( $components, $part[0] );
				}
			}
		...
		foreach ( $components as $comp ) {
			...
			if ( 'categories' == $comp ) {
					...
					if ( ! empty( $meta['sponsored_post'] ) && 1 == (int) $meta['sponsored_post'] ) {
						$cats = ( ! empty( $meta['sponsored_url'] )
								...
								. ( ! empty( $meta['sponsored_label'] )
									...
									: esc_html__( 'Sponsored content', 'catamaran' )
									)
								. ( ! empty( $meta['sponsored_url'] )
									? '</a>'
									: '</span>'
									);
						catamaran_show_layout( $cats, '<span class="post_meta_item post_sponsored">', '</span>');
					}
				}
				// Post categories
				$cats = get_post_type() == 'post'
								...
								: apply_filters( 'catamaran_filter_get_post_categories', '' );
				if ( ! empty( $cats ) ) {
					catamaran_show_layout( $cats, 
				...
				// Published date
				$dt = apply_filters( 'catamaran_filter_get_post_date', catamaran_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					catamaran_show_layout(
						...
						'<span class="post_meta_item post_date' . ( ! empty( $args['seo'] ) ? ' date published' : '' ) . '"'
							...
							. '>'
							. ( ! catamaran_is_single() ? '<a href="' . esc_url( get_permalink() ) . '">' : '' )
							. ( in_array( 'date', $components ) && in_array( 'modified', $components ) && get_post_type() == 'post' ? '<span class="post_meta_item_label">' . esc_html__( 'Published:', 'catamaran' ) . '</span>' : '' ),
						( ! catamaran_is_single() ? '</a>' : '' ) . '</span>'
					);
					$dt_last = $dt;
				}
			} elseif ( 'modified' == $comp && get_post_type() == 'post' ) {
				// Modified date
				$dt = apply_filters( 'catamaran_filter_get_post_modified_date', catamaran_get_date( get_post_modified_time( 'U' ), ! empty( $args['date_format'] ) ? $args['date_format'] : '', ! empty( $args['disable_date_diff'] ) ) );
				if ( ! empty( $dt ) && ( empty( $dt_last ) || $dt_last != $dt ) ) {
					catamaran_show_layout(
						$dt,
						'<span class="post_meta_item post_date' . ( ! empty( $args['seo'] ) ? ' date updated modified' : '' ) . '"'
							...
							. '>'
							. ( ! catamaran_is_single() ? '<a href="' . esc_url( get_permalink() ) . '">' : '' )
							. '<span class="post_meta_item_label">' . esc_html__( 'Updated:', 'catamaran' ) . '</span>',
						( ! catamaran_is_single() ? '</a>' : '' ) . '</span>'
					);
					$dt_last = $dt;
				}
				...
				if ( $author_id > 0 ) {
					...
					$author_avatar = ! empty( $args['author_avatar'] )
										? get_avatar( get_the_author_meta( 'user_email', $author_id ), apply_filters( 'catamaran_filter_author_avatar_size', 56, 'post_meta' ) * catamaran_get_retina_multiplier() ) 
										: '';
					echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
							. apply_filters( 'catamaran_filter_post_author_content',
								'<span class="post_author_by">' . esc_html__( 'By', 'catamaran' ) . '</span>'
								. ( ! empty( $author_avatar )
				...
				// Comments
				if ( !catamaran_is_single() || have_comments() || comments_open() ) {
					$post_comments = get_comments_number();
					echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
							. '<span class="post_meta_number">' . esc_html( catamaran_num2size( $post_comments ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _nx( 'Comment', 'Comments', $post_comments, 'Number of comments', 'catamaran' ) ) . '</span>' : '' )
						. '</a>';
				}
			...
			// Views
				...
				if ( function_exists( 'trx_addons_get_post_views' ) ) {
					...
					echo '<a href="' . esc_url( get_permalink() ) . '" class="post_meta_item post_meta_views trx_addons_icon-eye">'
							. '<span class="post_meta_number">' . esc_html( catamaran_num2size( $post_views ) ) . '</span>'
							. ( $args['show_labels'] ? '<span class="post_meta_label">' . esc_html( _n( 'View', 'Views', $post_views, 'catamaran' ) ) . '</span>' : '' )
						. '</a>';
				}
			...
			// Likes (Emotions)
				...
				if ( function_exists( 'trx_addons_get_post_likes' ) ) {
					...
					$liked = catamaran_get_cookie( 'trx_addons_likes' );
					...
					echo ( true == $emotions_allowed
							...
							: '<a href="#" role="button"'
								...
								. ' title="' . ( ! empty( $allow ) ? esc_attr( _x( 'Like', 'Link title', 'catamaran' ) ) : esc_attr( _x( 'Dislike', 'Link title', 'catamaran' ) ) ) . '"'
								. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
								. ' data-likes="' . esc_attr( $post_likes ) . '"'
								. ' data-title-like="' . esc_attr( _x( 'Like', 'Link title', 'catamaran' ) ) . '"'
								. ' data-title-dislike="' . esc_attr( _x( 'Dislike', 'Link title', 'catamaran' ) ) . '"'
								. '>'
							)
								. '<span class="post_meta_number">' . esc_html( catamaran_num2size( $post_likes ) )  . '</span>'
								. ( $args['show_labels']
									? '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? esc_html( _nx( 'Reaction', 'Reactions', $post_likes, 'Number of reactions', 'catamaran' ) )
											: esc_html( _nx( 'Like', 'Likes', $post_likes, 'Number of likes', 'catamaran' ) )
											)
									. '</span>'
									: '' )
							. '</a>';
				...
				// Socials share
				catamaran_show_share_links( apply_filters( 'catamaran_filter_share_links_args', 
					array(
						'type'      => $args['share_type'],
						'wrap'      => 'span',
						'direction' => $args['share_direction'],
						'caption'   => 'drop' == $args['share_type']
											? apply_filters( 'catamaran_filter_share_links_text', esc_html__( 'Share', 'catamaran' ) )
											: '',
				...
				// Edit page link
				edit_post_link( esc_html__( 'Edit', 'catamaran' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

			} else {
				// Custom counter
				do_action( 'catamaran_action_show_post_meta', $comp, get_the_ID(), $args );
			}
...
if ( ! function_exists( 'catamaran_show_post_featured' ) ) {
	/**
	 * Show a post featured block: image, video, audio, etc.
	 *
	 * @param array $args  Optional. Additional options to build a block layout.
	 */
	function catamaran_show_post_featured( $args = array() ) {
		$args = apply_filters( 'catamaran_filter_post_featured_args', array_merge(
			array(
				'popup'         => ! catamaran_is_single() && (int)catamaran_get_theme_option( 'video_in_popup', 0 ) > 0,  // Open video in popup
				'hover'         => catamaran_get_theme_option( 'image_hover' ),     // Hover effect
				'parallax'      => (int)catamaran_get_theme_option( 'single_parallax', 0 ), // Parallax speed. If 0 - no parallax effect is used
				...
				'show_no_image' => catamaran_is_on( catamaran_get_theme_setting( 'allow_no_image' ) ),  // Display 'no-image.jpg' if post haven't thumbnail
				'video'         => '',                                            // Video layout
				'autoplay'      => false,                                         // Autoplay video (if present)
				'seo'           => catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) ),     // Add SEO-snippets
		...
		$thumb_size        = ! empty( $args['thumb_size'] )
								? $args['thumb_size']
								: catamaran_get_thumb_size( is_attachment() || catamaran_is_single() ? 'full' : 'big' );
		$has_thumb         = has_post_thumbnail();
		$thumb_id          = 0;

		$parallax          = ! empty( $args['parallax'] ) && catamaran_is_singular('post') && ! in_array( $post_format, array( 'audio', 'video', 'gallery' ) )
								? $args['parallax']
								: 0;

		if ( 'audio' == $post_format ) {
			$audios = catamaran_get_post_audio_list();
			if ( ! empty( $audios[0]['cover'] ) ) {
				$has_thumb = true;
				$thumb_id  = catamaran_attachment_url_to_postid( $audios[0]['cover'] );
				if ( empty( $args['thumb_size'] ) ) {
					$ts = catamaran_detect_thumb_size( $audios[0]['cover'] );
					if ( ! empty( $ts ) ) {
						$thumb_size = $ts;
					}
				}
			...
			$videos = catamaran_get_post_video_list();
			if ( $post_meta === false ) {
			...
			if ( ( ! empty( $args['singular'] ) && ! empty( $post_meta['video_without_cover'] ) ) || ! empty( $args['autoplay'] ) ) {
				...
				$has_thumb = true;
				$thumb_id  = catamaran_attachment_url_to_postid( $videos[0]['image'] );
				if ( empty( $args['thumb_size'] ) ) {
					$ts = catamaran_detect_thumb_size( $videos[0]['image'] );
					if ( ! empty( $ts ) ) {
						$thumb_size = $ts;
					}
				}				
		...
		$no_image = ! empty( $args['show_no_image'] ) ? catamaran_get_no_image( '', true ) : '';
		...
		if ( $args['thumb_bg'] ) {
			if ( $has_thumb ) {
				...
				$image = catamaran_get_post_image();
				if ( ! empty( $image ) ) {
					$image = catamaran_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				...
				$args['thumb_bg_image'] = catamaran_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			} else {
				$args['thumb_bg'] = false;
			}
		}
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment
						...
						echo ( ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : '' )
							. ( ! empty( $args['css'] ) ? ' ' . catamaran_add_inline_css_class( $args['css'] ) : '' )
							. ( ! empty( $args['thumb_bg_class'] ) ? ' ' . esc_attr( $args['thumb_bg_class'] ) . ' ' . esc_attr( $args['thumb_bg_image'] ) : '' );
						?>
					">
				<?php
				do_action('catamaran_action_before_featured');
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
							get_the_ID(),
							$thumb_size,
							false,
							catamaran_is_on( catamaran_get_theme_option( 'seo_snippets' ) )
								? array( 'itemprop' => 'image' )
								: ''
					);
				}
				if ( catamaran_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				do_action('catamaran_action_after_featured');
				?>
				</div>
				<?php
				if ( has_excerpt() ) {
						...
						if ( ! empty( $videos ) && is_array( $videos ) ) {
							if ( count( $videos ) > 1 ) {
								...
								$args['post_info'] = apply_filters(
														'catamaran_filter_video_post_info',
				...
				if ( ( ( $has_thumb || ! empty( $args['show_no_image'] ) ) && ( ! catamaran_sc_layouts_showed( 'featured' ) || ! empty( $args['force'] ) ) )
						|| $show_video_player || ! empty( $videos )
						|| $show_audio_player
						|| $show_gallery
				) {
					$output = '<div class="'
									. apply_filters( 'catamaran_filter_post_featured_classes',
										'post_featured'
					...
					if ( $args['seo'] ) {
						$output .= ' itemscope="itemscope" itemprop="image" itemtype="' . esc_attr( catamaran_get_protocol( true ) ) . '//schema.org/ImageObject"';
					}
					...
					$args['data'] = apply_filters( 'catamaran_filter_post_featured_data', $args['data'], $args, 'singular' );
					if ( ! empty( $args['data'] ) && is_array( $args['data'] ) ) {
					...
					if ( $has_thumb && $args['seo'] ) {
						$catamaran_attr = catamaran_getimagesize( wp_get_attachment_url( $thumb_id ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr( $catamaran_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $catamaran_attr[1] ); ?>">
						<?php
					}
					...
					catamaran_show_post_avg( array_merge( $args, compact( 'post_format', 'has_thumb', 'thumb_id', 'thumb_size', 'videos', 'audios', 'gallery_images', 'post_meta' ) ) );
					// Put optional info block over the thumb
					catamaran_show_layout( $args['post_info'] );
			...
			if ( $has_thumb
				...
				$args['data'] = apply_filters( 'catamaran_filter_post_featured_data', $args['data'], $args, 'blog' );
				if ( ! empty( $args['data'] ) && is_array( $args['data'] ) ) {
					foreach( $args['data'] as $k => $v ) {
						$output .= ' data-' . esc_attr( $k ) . '="' . esc_attr( $v ) . '"';
					}
				...
				// Show hover
				if ( $show_hover && function_exists( 'catamaran_hovers_add_icons' ) ) {
					if ( ! empty( $args['hover'] ) ) {
						?>
						<div class="mask"></div>
						<?php
					}
					catamaran_hovers_add_icons(
						$args['hover'],
						array(
				...
				// Add audio, video or gallery
				catamaran_show_post_avg( array_merge( $args, compact( 'post_format', 'has_thumb', 'thumb_id', 'thumb_size', 'videos', 'audios' ) ) );
...
if ( ! function_exists( 'catamaran_show_post_avg' ) ) {
	/**
	 * Show an audio, a video or a gallery inside the featured image.
	 *
	 * @param array $args  Options from a parent function catamaran_show_featured_image() with additional parameters
	 *                     (for example 'videos' - a list of the videos from the current post to display)
	 */
	function catamaran_show_post_avg( $args ) {

		if ( ! $args['thumb_only'] && ( in_array( $args['post_format'], array( 'video', 'audio', 'gallery' ) ) || ! empty( $args['video'] ) ) ) {
			
			$post_content = catamaran_get_post_content();
			...
			if ( 'video' == $args['post_format'] || ! empty( $args['video'] ) ) {
				if ( ! empty( $args['singular'] ) 
					...
					<div class="post_video_list">
						<?php
						catamaran_show_layout( trx_addons_sc_widget_video_list( array(
					...
					if ( ! empty( $args['video'] ) ) {
						...
						$video = catamaran_get_post_video_list_first( true );
					} else if ( catamaran_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
						$video = catamaran_get_post_video( $post_content, false );
					}
					if ( empty( $args['singular'] ) ) {
						if ( empty( $video ) ) {
							$video = catamaran_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = catamaran_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = catamaran_array_get_first( $videos, false );
								}
							}
						}
					}
						...
						if ( $args['has_thumb'] && ! empty( $args['popup'] ) && function_exists( 'trx_addons_get_video_layout' ) ) {
							...
							if ( ! empty( $popup[0] ) && ! empty( $popup[1] ) ) {
								if ( preg_match( '/<a .*<\/a>/', $popup[0], $matches ) && ! empty( $matches[0] ) ) {
									$video_out = true;
									?>
									<div class="post_video_hover post_video_hover_popup"><?php catamaran_show_layout( $matches[0] ); ?></div>
									<?php
									catamaran_show_layout($popup[1]);
								}
							}
						}
						if ( ! $video_out ) {
							if ( ! empty( $args['autoplay'] ) ) {
								$video = catamaran_make_video_autoplay( $video, true );
							} else if ( ! empty( $args['has_thumb'] ) ) {
								$video = catamaran_make_video_autoplay( $video );
								?>
								<div class="post_video_hover" data-video="<?php echo esc_attr( $video ); ?>"></div>
								<?php
							}
							do_action( 'catamaran_action_before_single_post_video', $args );
							?>
							...
							?>">
								...
								if ( empty( $args['has_thumb'] ) || ! empty( $args['autoplay'] ) ) {
									catamaran_show_layout( $video );
								}
								?>
							</div>
							<?php
							do_action( 'catamaran_action_after_single_post_video', $args );
						}
					}
				}
				...
				if ( ! empty( $args['audios'][0] ) ) {
					...
					$audio             = catamaran_get_post_audio_list_first( true );
				}
				if ( empty( $args['singular'] ) ) {
					if ( empty( $audio ) && catamaran_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
						$audio = catamaran_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = catamaran_get_post_iframe( $post_content, false );
						}
					}
					...
					if ( empty( $audio ) && catamaran_get_theme_option( 'blog_content', 'excerpt' ) != 'fullpost' ) {
						...
						if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
							...
							if ( ! empty( $audios ) && is_array( $audios ) ) {
								$audio = catamaran_array_get_first( $audios, false );
							}
						}
					}
				}
					...
					">
						...
						if ( empty( $args['singular'] ) && empty( $media_author ) && empty( $media_title ) ) {
							if ( strpos( $audio, '<audio' ) !== false ) {
								$media_author = catamaran_get_tag_attrib( $audio, '<audio>', 'data-author' );
								$media_title  = catamaran_get_tag_attrib( $audio, '<audio>', 'data-caption' );
							}
							if ( empty( $media_author) &&  empty( $media_title) ) {
								$media = urldecode( catamaran_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								if ( ! empty( $media ) ) {
									...
									// Parse tag params
									$media_author = strip_tags( catamaran_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
									$media_title  = strip_tags( catamaran_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );
						...
						if ( ! empty( $media_author) || ! empty( $media_title) ) {
							...
							<div class="post_info_audio">
								...
								if ( ! empty( $media_author ) ) {
									catamaran_show_layout( $media_author, '<div class="post_audio_author">', '</div>' );
								}
								if ( ! empty( $media_title ) ) {
									catamaran_show_layout( $media_title, '<h5 class="post_audio_title">', '</h5>' );
								}
								if ( ! empty( $media_description ) ) {
									catamaran_show_layout( $media_description, '<div class="post_audio_description">', '</div>' );
								}
								?>
							</div>
							<?php
						...
						// Display audio
						catamaran_show_layout( $audio, '<div class="audio_wrap">', '</div>' );
						?>
					</div>
					<?php
				}
				...
				$slider_args['slides_ratio'] = ! empty( $args['thumb_ratio'] ) && ! in_array( $args['thumb_ratio'], array('none', 'masonry') )
													? $args['thumb_ratio']
													: '16:9';
				$output = catamaran_get_slider_layout( apply_filters( 'catamaran_filter_post_slider_args', $slider_args ) );
				if ( '' != $output ) {
					catamaran_show_layout( $output );
				}
			}
...
if ( ! function_exists( 'catamaran_is_with_featured_image' ) ) {
	/**
	 ...
	 */
	function catamaran_is_with_featured_image( $content, $disabled=array() ) {
...
if ( ! function_exists( 'catamaran_get_no_image' ) ) {
	/**
	 ...
	 */
	function catamaran_get_no_image( $no_image = '', $need = false, $return_url = true ) {
		static $no_image_url = '', $no_image_dir = '';
		$img = catamaran_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || catamaran_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( $return_url ) {
				if ( empty( $no_image_url ) ) {
					$no_image_url = catamaran_get_file_url( 'images/no-image.jpg' );
				}
				$img = $no_image_url;
			} else {
				if ( empty( $no_image_dir ) ) {
					$no_image_dir = catamaran_get_file_dir( 'images/no-image.jpg' );
				}
				$img = $no_image_dir;
			}
		}
...
if ( ! function_exists( 'catamaran_add_bg_in_post_nav' ) ) {
	/**
	 ...
	 */
	function catamaran_add_bg_in_post_nav() {
		if ( ! catamaran_is_single() || ! catamaran_get_theme_setting( 'thumbs_in_navigation' ) ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), catamaran_get_thumb_size( 'med' ) );
			...
			if ( ! empty( $img ) ) {
				...
				$css .= apply_filters( 'catamaran_filter_post_navigation_previous_css',
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), catamaran_get_thumb_size( 'med' ) );
				$img = empty( $img[0] ) ? '' : $img[0];
			} else {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
				...
				$css .= apply_filters( 'catamaran_filter_post_navigation_next_css',
		...
		catamaran_add_inline_css( $css );
	}
}

if ( ! function_exists( 'catamaran_show_related_posts_callback' ) ) {
	add_action( 'catamaran_action_related_posts', 'catamaran_show_related_posts_callback' );
	/**
	 * Callback for action 'Related posts' to display a related posts in the different places of the theme.
	 *
	 * @hooked 'catamaran_action_related_posts'
	 */
	function catamaran_show_related_posts_callback() {
		if ( catamaran_is_single() && ! apply_filters( 'catamaran_filter_show_related_posts', false ) ) {
			$catamaran_related_posts   = (int) catamaran_get_theme_option( 'related_posts' );
			$catamaran_related_columns = (int) catamaran_get_theme_option( 'related_columns' );
			$catamaran_related_style   = catamaran_get_theme_option( 'related_style', 'classic' );
			if ( (int) catamaran_get_theme_option( 'show_related_posts' ) && $catamaran_related_posts > 0 ) {
				catamaran_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $catamaran_related_posts ) ),
						'columns'        => max( 1, min( 6, $catamaran_related_posts, $catamaran_related_columns ) ),
					),
					$catamaran_related_style
				);
			}
		}
	}
...
if ( ! function_exists( 'catamaran_show_related_posts' ) ) {
	/**
	 ...
	 */
	function catamaran_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : catamaran_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int)catamaran_get_theme_option( 'related_slider', 0 ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = catamaran_get_theme_option( 'related_position', 'below_content' );
			$related_style = catamaran_get_theme_option( 'related_style', 'classic' );
			$related_tag = strpos( $related_position, 'inside' ) === 0 ? 'h5' : 'h3';
			if ( in_array( $related_position, array( 'inside_left', 'inside_right' ) ) ) {
				$columns = 1;
			}

			do_action( 'catamaran_action_before_related_wrap' );
			?>
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				...
				<?php do_action( 'catamaran_action_before_related_wrap_title' ); ?>

				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						echo esc_html( $title );
					} else {
						esc_html_e( 'You May Also Like', 'catamaran' );
					}
				?></<?php echo esc_html( $related_tag ); ?>><?php

				do_action( 'catamaran_action_after_related_wrap_title' );

				if ( $args['slider'] ) {
					$slider_args                      = $args;
					$slider_args['count']             = max(1, $query->found_posts);
					$slider_args['slides_min_width']  = 250;
					$slider_args['slides_space']      = (int)catamaran_get_theme_option( 'related_slider_space', 30 );
					$slider_args['slider_controls']   = catamaran_get_theme_option( 'related_slider_controls', 'none' );
					$slider_args['slider_pagination'] = catamaran_get_theme_option( 'related_slider_pagination', 'bottom' );
					$slider_args                      = apply_filters( 'catamaran_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					catamaran_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						if ( ! apply_filters( 'catamaran_filter_related_post_showed', false, $args, $style ) ) {
							get_template_part( apply_filters( 'catamaran_filter_get_template_part', 'templates/related-posts', $style ), $style );
						}
						?></div><?php
					}
				?></div><?php
				if ( $args['slider'] ) {
					catamaran_get_slider_wrap_end('related_posts_wrap', $slider_args);
					?></div><?php
				}
				wp_reset_postdata();
				?>
			...
			<?php
			do_action( 'catamaran_action_after_related_wrap' );
		}
	}
}

if ( ! function_exists( 'catamaran_is_blog_style_use_masonry' ) ) {
	/**
	 ...
	 */
	function catamaran_is_blog_style_use_masonry( $style ) {
		$blog_styles = catamaran_storage_get( 'blog_styles' );
		return ! empty( $blog_styles[ $style ][ 'scripts' ] ) && in_array( 'masonry', (array) $blog_styles[ $style ][ 'scripts'] );
	}
}

if ( ! function_exists( 'catamaran_show_filters' ) ) {
	/**
	 ...
	 *                                ]
	 */
	function catamaran_show_filters( $args = array() ) {
		$args = array_merge(
			array(
				'post_type'  => '',
				'taxonomy'   => '',
		...
		if ( is_array( $terms ) && count( $terms ) > 0 ) {
			$tabs[ $args['parent_cat'] ] = esc_html__( 'All', 'catamaran' );
		...
		if ( count( $tabs ) > 0 ) {
			$filters_ajax   = catamaran_get_theme_setting( 'blog_filters_use_ajax' );
			$filters_active = $args['parent_cat'];
			$filters_id     = 'catamaran_filters';
			?>
			<div class="catamaran_tabs catamaran_tabs_ajax catamaran_filters">
				<ul class="catamaran_tabs_titles">
					<?php
					foreach ( $tabs as $tab_id => $tab_title ) {
						?><li>
							<a href="<?php echo esc_url( catamaran_get_hash_link( sprintf( '#%1$s_%2$s_content', $filters_id, $tab_id ) ) ); ?>"
				...
				foreach ( $tabs as $tab_id => $tab_title ) {
					...
					<div id="<?php echo esc_attr( sprintf( '%1$s_%2$s_content', $filters_id, $tab_id ) ); ?>"
						class="catamaran_tabs_content"
						data-blog-template="<?php echo esc_attr( catamaran_storage_get( 'blog_template' ) ); ?>"
					...
					>
						<?php
						if ( $tab_need_content ) {
							catamaran_show_posts( array_merge( $args, array( 'cat' => $tab_id ) ) );
						}
						?>
					</div>
					<?php
			...
			catamaran_show_posts( array_merge( $args, array( 'cat' => $args['parent_cat'] ) ) );
		}
	}
}

if ( ! function_exists( 'catamaran_show_posts' ) ) {
	/**
	 ...
	 */
	function catamaran_show_posts( $args = array() ) {
		...
		$args = array_merge(
			array(
				...
				'parent_cat'     => 0,
				'cat'            => 0,
				'posts_per_page' => (int) catamaran_get_theme_option( 'posts_per_page' ),
				'page'           => 1,
				'sticky'         => false,
				'blog_style'     => '',
				'echo'           => true,
		...
		$blog_style   = empty( $args['blog_style'] ) ? catamaran_get_theme_option( 'blog_style' ) : $args['blog_style'];
		...
		if ( strpos( $style, 'blog-custom-' ) === 0 ) {
			$custom_blog_id   = catamaran_get_custom_blog_id( $style );
			$custom_blog_meta = catamaran_get_custom_layout_meta( $custom_blog_id );
			if ( ! empty( $custom_blog_meta['margin'] ) ) {
				catamaran_add_inline_css( sprintf( '.page_content_wrap{padding-top:%s}', esc_attr( catamaran_prepare_css_value( $custom_blog_meta['margin'] ) ) ) );
			}
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = catamaran_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
		...
		if ( catamaran_is_blog_style_use_masonry( $style ) ) {
			catamaran_lazy_load_off();
		}

		// Show posts
		$class = apply_filters( 'catamaran_filter_posts_container_classes', 'posts_container'
				. sprintf( ' %1$s_wrap %1$s_%2$d', $style, $columns )
				. ( ! catamaran_is_off( $custom_style )
					? sprintf( ' %s_wrap', $custom_style ) . ( 'masonry' == $custom_style ? sprintf( ' masonry_%d', $columns ) : '' )
					: ( catamaran_is_blog_style_use_masonry( $style )
						?  sprintf( ' masonry_wrap masonry_%1$d', $columns )
						: ( $columns > 1 || apply_filters( 'catamaran_filter_columns_wrap_for_single_column', false )
		...
		if ( $args['sticky'] ) {
			...
			if ( (int)catamaran_get_theme_option( 'first_post_large', 0 ) > 0 && ! is_paged() && ! in_array( catamaran_get_theme_option( 'body_style' ), array( 'fullwide', 'fullscreen' ) ) && have_posts() ) {
				the_post();
				get_template_part( apply_filters( 'catamaran_filter_get_template_part', 'templates/content', 'excerpt' ), 'excerpt' );
			}
			?>
		...
		while ( have_posts() ) {
			...
			get_template_part( apply_filters( 'catamaran_filter_get_template_part', $args['sticky'] && is_sticky() ? 'templates/content-sticky' : catamaran_blog_item_get_template( $blog_style ) ) );
		}
...
if ( ! function_exists( 'catamaran_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_catamaran_ajax_get_posts', 'catamaran_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_catamaran_ajax_get_posts', 'catamaran_ajax_get_posts_callback' );
	/**
	 * Return a portfolio posts layout to AJAX requests when switching tabs and pages.
	 *
	 * @hooked 'wp_ajax_catamaran_ajax_get_posts'
	 * @hooked 'wp_ajax_nopriv_catamaran_ajax_get_posts'
	 */
	function catamaran_ajax_get_posts_callback() {
		...
		$response = array(
			...
			'data'  => catamaran_show_posts(
				array(
					...
					'blog_style' => str_replace( array( '.', '/', '\\' ), '', catamaran_esc( trim( wp_unslash( $_REQUEST['blog_style'] ) ) ) ),
					'echo'       => false,
				)
			),
			'css'  => catamaran_get_inline_css(),
		);

		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'catamaran' );
		}
...
if ( ! function_exists( 'catamaran_show_pagination' ) ) {
	/**
	 ...
	 */
	function catamaran_show_pagination( $args = array() ) {
		global $wp_query;
		$pagination = ! empty( $args[ 'pagination' ] )
						? $args[ 'pagination' ]
						: catamaran_get_theme_option( 'blog_pagination' );
		$prefix     = ! empty( $args[ 'class_prefix' ] )
						? $args[ 'class_prefix' ]
						: 'nav';
		if ( 'pages' == $pagination ) {
			catamaran_show_layout( str_replace( "\n", '', get_the_posts_pagination(
				apply_filters( 'catamaran_filter_get_the_posts_pagination_args', array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'catamaran' ),
					'next_text'          => esc_html__( '>', 'catamaran' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'catamaran' ) . ' </span>',
				) )
			) ) );
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				<div class="<?php echo esc_attr( $prefix ); ?>-links-more
					...
					?>
				">
					<a class="<?php echo esc_attr( apply_filters( 'catamaran_filter_load_more_class', "{$prefix}-load-more" ) ); ?>"
						href="#" role="button" 
						data-page="<?php echo esc_attr( $page_number ); ?>" 
						data-max-page="<?php echo esc_attr( $wp_query->max_num_pages ); ?>"
						<?php do_action( 'catamaran_action_load_more_data' ); ?>
						><span><?php echo esc_html( apply_filters( 'catamaran_filter_load_more_text', __( 'Load more', 'catamaran' ) ) ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="<?php echo esc_attr( $prefix ); ?>-links-old">
				<span class="<?php echo esc_attr( $prefix ); ?>-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'catamaran' ) : esc_html__( 'Newest posts', 'catamaran' ) ); ?></span>
				<span class="<?php echo esc_attr( $prefix ); ?>-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'catamaran' ) : esc_html__( 'Older posts', 'catamaran' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'catamaran_single_comments_field' ) ) {
	/**
	 ...
	 */
	function catamaran_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'catamaran_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'catamaran_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
