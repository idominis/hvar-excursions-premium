 ...
 *
 * @package CATAMARAN
 * @since CATAMARAN 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'CATAMARAN_THEME_FREE' ) ) {
	define( 'CATAMARAN_THEME_FREE', false );
}
if ( ! defined( 'CATAMARAN_THEME_FREE_WP' ) ) {
	define( 'CATAMARAN_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'CATAMARAN_THEME_ALLOW_UPDATE' ) ) {
	define( 'CATAMARAN_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'CATAMARAN_ALLOW_SKINS' ) ) {
	define( 'CATAMARAN_ALLOW_SKINS', true );
}
if ( ! defined( 'CATAMARAN_DEFAULT_SKIN' ) ) {
	define( 'CATAMARAN_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'CATAMARAN_REMEMBER_SKIN' ) ) {
	define( 'CATAMARAN_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['CATAMARAN_STORAGE'] = array(
...
if ( ! function_exists( 'catamaran_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function catamaran_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = catamaran_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'catamaran_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'catamaran_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function catamaran_trx_updater_get_theme_info( $info ) {
		$engine = catamaran_storage_get( 'engine' );
...
if ( ! function_exists( 'catamaran_original_theme_slug' ) ) {
	add_filter( 'catamaran_filter_original_theme_slug', 'catamaran_original_theme_slug' );
	/**
	 ...
	 *
	 * @hooked 'catamaran_filter_original_theme_slug'
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function catamaran_original_theme_slug( $theme_slug = '' ) {
		return 'catamaran';
	}
}
...
if ( ! function_exists( 'catamaran_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'catamaran_importer_set_options', 9 );
	/**
	 ...
	 */
	function catamaran_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( catamaran_get_protocol() . ':' . catamaran_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( catamaran_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'catamaran_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'catamaran_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function catamaran_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( catamaran_get_protocol() . ':' . catamaran_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'catamaran_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'catamaran_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function catamaran_customizer_theme_setup1() {
		catamaran_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'catamaran_filter_allow_fullscreen' return false)
