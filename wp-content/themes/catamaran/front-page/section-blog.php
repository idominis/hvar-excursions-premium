<div class="front_page_section front_page_section_blog<?php
	$catamaran_scheme = catamaran_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $catamaran_scheme ) && ! catamaran_is_inherit( $catamaran_scheme ) ) {
		echo ' scheme_' . esc_attr( $catamaran_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( catamaran_get_theme_option( 'front_page_blog_paddings' ) );
	if ( catamaran_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$catamaran_css      = '';
		$catamaran_bg_image = catamaran_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$catamaran_anchor_icon = catamaran_get_theme_option( 'front_page_blog_anchor_icon' );
	$catamaran_anchor_text = catamaran_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( catamaran_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$catamaran_css      = '';
			$catamaran_bg_mask  = catamaran_get_theme_option( 'front_page_blog_bg_mask' );
			$catamaran_bg_color_type = catamaran_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $catamaran_bg_color_type ) {
				$catamaran_bg_color = catamaran_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$catamaran_caption = catamaran_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $catamaran_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $catamaran_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $catamaran_caption, 'catamaran_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$catamaran_description = catamaran_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $catamaran_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $catamaran_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $catamaran_description ), 'catamaran_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! catamaran_exists_trx_addons() ) {
						...
						catamaran_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
